...
2. 导航到`ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components/card`并视图`.content.xml`文件。
   ...
   属性`sling:resourceSuperType`指向`wknd-spa-angular/components/image`，指示`Card`组件将继承WKND SPA图像组件的所有功能。
...
7. Inspect文件`card.component.ts`。
   ...
   查看类中`src`、`alt`和`title`的三个`@Input`参数。 这些是将映射到Angular组件的AEM组件中的预期JSON值。
...
8. 打开文件`card.component.html`:
   ...
   在此示例中，我们选择通过简单传递`card.component.ts`中的`@Input`参数来重新使用现有的Angular图像组件`app-image`。 在教程的稍后部分，将添加和显示其他属性。
...
要最终将组件对话框中的值暴露给Angular组件，我们需要更新Sling Model，该Sling Model填充`Card`组件的JSON。 我们还有机会实施两个业务逻辑：
...
2. 在`ui.frontend/src/app/components/card/card.component.ts`打开`card.component.ts`。 添加附加的`@Input`注释以捕获新模型：
...
3. 添加方法，用于检查行动动员是否就绪，并根据`cardLastModified`输入返回日期/时间字符串：
...
4. 打开`card.component.html`并添加以下标记以显示标题、行动动员和上次修改日期：
   ...
   已在`card.component.scss`添加了Sass规则，以设置标题的样式、行动动员和上次修改日期。

   >[!NOTE]
   >
   > 您可以在此处视图已完成的[Angular卡组件代码](https://github.com/adobe/aem-guides-wknd-spa/tree/Angular/extend-component-solution/ui.frontend/src/app/components/card)。
