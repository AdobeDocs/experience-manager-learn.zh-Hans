...
description: 了解如何使用SPA Editor JS SDK将Angular组件映射到Adobe Experience Manager(AEM)组件。 组件映射使用户能够在AEM SPA编辑器中对SPA组件进行动态更新，这与传统的AEM创作类似。
sub-product: 站点
feature: SPA Editor
topics: development
doc-type: tutorial
...
了解如何使用SPA Editor JS SDK将Angular组件映射到Adobe Experience Manager(AEM)组件。 组件映射使用户能够在AEM SPA编辑器中对SPA组件进行动态更新，这与传统的AEM创作类似。
...
4. 要检查的第一个区域是位于~行35的`class TextComponent`:
   ...
   [@Input()](https://angular.io/api/core/Input) 修饰符用于声明通过映射的JSON对象设置值的字段，该对象已在之前审阅。

   `@HostBinding('innerHtml') get content()` 是根据值公开创作的文本内容的方 `this.text`法如果内容为富文本（由`this.richText`标志确定），则会绕过Angular的内置安全性。 Angular的[DomSanitizer](https://angular.io/api/platform-browser/DomSanitizer)用于“清除”原始HTML并防止跨站点脚本漏洞。 该方法使用[@HostBinding](https://angular.io/api/core/HostBinding)解码器绑定到`innerHtml`属性。

5. 接下来，检查位于~行24的`TextEditConfig`:
...
6. 最后查看~line 53的`MapTo`调用：

   ```js
   MapTo('wknd-spa-angular/components/text')(TextComponent, TextEditConfig );
   ```

   **aem** SPA Editor JS SDK()提供`@adobe/cq-angular-editable-components`MapTois。路径`wknd-spa-angular/components/text`表示AEM组件的`sling:resourceType`。 此路径与前面观察的JSON模型公开的`:type`匹配。 **映** 射解析JSON模型响应，并将正确的值传递 `@Input()` 给SPA组件的变量。

   可在`ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components/text`找到AEM `Text`组件定义。

7. 尝试修改位于`ui.frontend/src/mocks/json/en.model.json`的&#x200B;**en.model.json**&#x200B;文件。
   ...
   返回浏览器查看&#x200B;**webpack dev server**&#x200B;提供的效果：
...
8. Inspect **text.component.html**，地址为`ui.frontend/src/app/components/text/text.component.html`。
   ...
   此文件为空，因为组件的整个内容将由`innerHTML`属性设置。

9. Inspect位于`ui.frontend/src/app/app.module.ts`的&#x200B;**app.module.ts**。
   ...
   **TextComponent**&#x200B;未显式包含，而是通过AEM SPA Editor JS SDK提供的&#x200B;**AEMResponsiveGridComponent**&#x200B;动态地包含。 因此，必须在&#x200B;**app.module.ts**&#39; [entryComponents](https://angular.io/guide/entry-components)数组中列出。

## 创建图像组件

然后，创建映射到AEM [图像组件](https://docs.adobe.com/content/help/zh-Hans/experience-manager-core-components/using/components/image.html)的`Image`角度组件。 `Image`组件是&#x200B;**content**&#x200B;组件的另一个示例。
...
2. 返回IDE并打开位于`ui.frontend/src/mocks/json/en.model.json`的`en.model.json`。 由于这是我们项目的新组件，因此我们需要“模拟”图像JSON。
...
3. 添加要由组件显示的库存照片。
   ...
   在`ui.frontend/src/mocks`下新建一个名为&#x200B;**images**&#x200B;的文件夹。 下载[adobestock-140634652.jpeg](assets/map-components/adobestock-140634652.jpeg)并将其放入新创建的&#x200B;**images**&#x200B;文件夹。 如果需要，可随意使用您自己的图像。

### 实现图像组件

1. 如果启动，请停止&#x200B;**Webpack dev server**。
2. 通过从`ui.frontend`文件夹中运行Angular CLI `ng generate component`命令新建一个映像组件：

   ```shell
   $ ng generate component components/image
   ```

3. 在IDE中，打开&#x200B;**image.component.ts**（位于`ui.frontend/src/app/components/image/image.component.ts`），然后按如下方式进行更新：
   ...
   `@Input()` 的 `src`、 `alt`以及 `title` 是从JSON API映射的属性。
...
5. 打开&#x200B;**image.component.scss**&#x200B;并按如下方式更新它：
   ...
   >[!NOTE]
   >
   > `:host-context`规则是AEM SPA编辑器占位符的&#x200B;**critical**，以正确运行。 所有要在AEM页面编辑器中创作的SPA组件至少需要此规则。

6. 打开`app.module.ts`并将`ImageComponent`添加到`entryComponents`阵列：

   ```js
   entryComponents: [TextComponent, PageComponent, ImageComponent],
   ```

   与`TextComponent`一样，`ImageComponent`是动态加载的，并且必须包含在`entryComponents`数组中。
...
7. 开始&#x200B;**webpack dev server**&#x200B;以查看`ImageComponent`渲染。
   ...
   *已添加到SPA的图像*
   ...
   >[!NOTE]
   >
   > **奖金挑战**:实现一种新方法，将值作为 `title` 字幕显示在图像下方。

## 更新AEM中的策略

`ImageComponent`组件仅在&#x200B;**webpack dev server**&#x200B;中可见。 然后，将更新的SPA部署到AEM并更新模板策略。

1. 停止&#x200B;**webpack dev server**&#x200B;和从项目的&#x200B;**root**&#x200B;中，使用您的Maven技能将更改部署到AEM::
...
2. 从AEM开始屏幕导航到&#x200B;**[!UICONTROL 工具]** > **[!UICONTROL 模板]** > **[WKNDSPA角](http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf/wknd-spa-angular)**。
...
4. 在&#x200B;**允许的组件** > **WKND SPA角度——内容**&#x200B;下，检查&#x200B;**图像**&#x200B;组件：

   ![已选择图像组件](assets/map-components/check-image-component.png)

   在&#x200B;**默认组件** > **添加映射**&#x200B;下，选择&#x200B;**图像- WKND SPA角——内容**&#x200B;组件：
...
1. 在IDE中，打开位于`ui.frontend/src/app/components/responsive-grid`的&#x200B;**responsive-grid.component.ts**:
...
3. 返回至[http://localhost:4502/editor.html/content/wknd-spa-angular/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-angular/us/en/home.html)。 添加其他&#x200B;**Image**&#x200B;组件，并尝试使用&#x200B;**布局**&#x200B;选项重新调整它们的大小：
...
6. 您应该能够使用`Text`组件的响应功能和更新的富文本策略来创作视图，如下所示：
...
[导航和路由](navigation-routing.md) -了解如何通过使用SPA Editor SDK映射到AEM Pages来支持SPA中的多个视图。动态导航是使用角度路由器实现的，并添加到现有的头组件中。
