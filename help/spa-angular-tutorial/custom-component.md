...
2. 导航到`ui.apps/src/main/content/jcr_root/apps/wknd-spa-angular/components`并新建一个名为`custom-component`的文件夹。
...
3. 更新`CustomComponent.java`，以扩展`ComponentExporter`接口：
   ...
   `CustomComponent`接口包括单个getter方法`getMessage()`。 这是通过JSON模型显示创作对话框值的方法。 在JSON模型中只导出参数为`()`的getter方法。

4. 打开`core/src/main/java/com/adobe/aem/guides/wknd/spa/angular/core/models/impl/CustomComponentImpl.java`处的`CustomComponentImpl.java`。
...
6. 将`getExportedType()`方法添加到`CustomComponentImpl`类以返回组件资源类型：
   ...
   实现`ComponentExporter`接口时需要此方法，并将显示允许映射到角度组件的资源类型。
...
4. 打开`cutom.component.html`并观察`{{message}}`的值将显示在`<h2>`标记的侧面。
5. 打开`custom.component.css`并添加以下规则：
   ...
   为了使AEM编辑器占位符在组件为空时正确显示，`:host-context`或其他`<div>`必须设置为`display: block;`。
...
1. 导航至[http://localhost:4502/system/console/status-slingmodels](http://localhost:4502/system/console/status-slingmodels)验证新Sling Model的注册。
   ...
   您应当看到以上两行，它们指示`CustomComponentImpl`与`wknd-spa-angular/components/custom-component`组件关联，并且它已通过Sling Model Exporter注册。
