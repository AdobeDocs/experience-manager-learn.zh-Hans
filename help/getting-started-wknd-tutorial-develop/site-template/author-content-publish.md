...
description: 使用Adobe Experience Manager AEM中的页面编辑器来更新网站内容。 了解如何使用组件促进创作。 了解AEM创作环境和发布环境之间的区别，并了解如何将更改发布到活动站点。
...
AEM提供了一组 [核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html) 即生产就绪可供使用的产品。 此 **核心组件** 从基本元素开始，例如 [文本](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/text.html) 和 [图像](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/image.html) 到更复杂的UI元素，如 [轮播](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/carousel.html).

接下来，使用AEM页面编辑器创作一些组件。

1. 导航至 **《你好世界》** 在上一个练习中创建的页面。
1. 确保您位于 **编辑** 模式，在左边栏中单击 **组件** 图标。

   ![页面编辑器边栏](assets/author-content-publish/page-editor-siderail.png)

   这将打开组件库，并列出可在页面上使用的可用组件。

1. 向下滚动并 **拖放** a **文本(v2)** 组件添加到页面的主要可编辑区域。

   ![拖放文本组件](assets/author-content-publish/drag-drop-text-cmp.png)

1. 单击 **文本** 要高亮显示的组件，然后单击 **扳手** 图标 ![“扳手”图标](assets/author-content-publish/wrench-icon.png) 以打开组件的对话框。 输入一些文本并将更改保存到对话框。

   ![富文本组件](assets/author-content-publish/rich-text-populated-component.png)

   此 **文本** 组件现在应在页面上显示富文本。

1. 重复上述步骤，但拖动 **图像(v2)** 组件添加到页面上。 打开 **图像** 组件的对话框。

1. 在左边栏中，切换到 **资产查找器** 通过单击 **资产** 图标 ![资产图标](assets/author-content-publish/asset-icon.png).
1. **拖放** 将图像放入组件对话框，然后单击 **完成** 以保存更改。

   ![将资源添加到对话框](assets/author-content-publish/add-asset-dialog.png)

1. 请注意，页面上存在组件，例如 **标题**， **导航**， **Search** 已修复。 这些区域配置为页面模板的一部分，无法在单个页面上修改。 下一章将对此进行详细探讨。

您可以随意尝试一些其他组件。 关于每个项目的文档 [可以在此处找到核心组件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html). 有关以下内容的详细视频系列： [可以在此处找到页面创作](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/page-authoring/aem-sites-authoring-overview.html).

## 发布更新 {#publish-updates}

AEM环境在 **作者服务** 和 **发布服务**. 在本章中，我们对以下网站进行了多处修改： **作者服务**. 为了让网站访客查看更改，我们需要将它们发布到 **发布服务**.

![高级图表](assets/author-content-publish/author-publish-high-level-flow.png)

*从创作到发布的高级别内容流*

**1.** 内容作者对网站内容进行更新。 可以预览、审核和批准更新以将其推送到实时。
...
1. 下 **选项** 将默认设置保留为 **Publish** 并安排它用于 **现在**. 单击&#x200B;**下一步**。
...
1. 在 **工作流** 步骤，输入 **工作流标题**. 这可以是任何文本，并且以后作为审核跟踪的一部分将会很有用。 输入“Initial publish”并单击 **Publish**.
...
1. 添加 `/content/wknd.html` 到发布URL，以便最终URL如下所示： `https://publish-pYYYY-eXXXX.adobeaemcloud.com/content/wknd.html`.
   ...
   >[!NOTE]
   >
   > 更改 `wknd.html` 以匹配您的网站名称，如果您在 [站点创建](create-site.md).
...
1. 返回到 **AEM创作服务** 并在页面编辑器中执行一些其他内容更改。
1. 通过单击 **页面属性** 图标> **发布页面**

   ![直接发布](assets/author-content-publish/page-editor-publish.png)

1. 返回到 **AEM发布服务** 以查看更改。 您很有可能 **非** 立即查看更新。 这是因为 **AEM发布服务** 包含 [通过Apache Web服务器和CDN进行缓存](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/content-delivery/caching.html). 默认情况下，HTML内容的缓存时间约为5分钟。

1. 要绕过缓存以进行测试/调试，只需添加查询参数，如 `?nocache=true`. URL如下所示 `https://publish-pYYYY-eXXXX.adobeaemcloud.com/content/wknd/en/article/hello-world.html?nocache=true`. 有关缓存策略和可用配置的更多详细信息 [可在此处找到](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/content-delivery/overview.html).

1. 您还可以在Cloud Manager中找到发布服务的URL。 导航至 **Cloud Manager项目** > **环境** > **环境**.
...
在现实实施中，通常在创建站点之前规划具有模型和UI设计的站点。 了解如何使用Adobe XD UI工具包设计和加速您在中的Adobe Experience Manager Sites实施 [使用Adobe XD进行UI规划](./ui-planning-adobe-xd.md).

想要继续探索AEM Sites的功能？ 请随时跳到上一章 [页面模板](./page-templates.md) 以了解页面模板和页面之间的关系。
