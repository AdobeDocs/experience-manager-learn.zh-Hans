...
_如果系统中同时安装了Java 8和Java 11，则VS代码测试运行程序在执行测试时可能会选择较低的Java运行时，从而导致测试失败。如果发生这种情况，请卸载Java 8._
...
1. 通过在Java包文件夹结构中在`src/test/java`下创建一个新的Java类（该类反映要测试的Java类的位置），为`BylineImpl.java`创建一个单元测试。
...
2.同时，区分测试文件    设备测试文件`BylineImplTest.java`上的`Test`后缀是一种惯例，它允许我们
1.轻松将其标识为_`BylineImpl.java`的测试文件_
2.但是，还应将测试文件_与要测试的类_&#x200B;区分开， `BylineImpl.java`

## 查看BylineImplTest.java {#reviewing-bylineimpltest-java}

此时，JUnit测试文件是空的Java类。 使用以下代码更新文件：
...
1. 第一个方法`public void setUp() { .. }`用JUnit的`@BeforeEach`进行注释，它指示JUnit测试运行器在运行此类中的每个测试方法之前执行此方法。 这为初始化所有测试所需的常见测试状态提供了方便。

2. 后续方法是测试方法，其名称以约定`test`为前缀，并用`@Test`注释标记。 请注意，默认情况下，我们的所有测试都设置为失败，因为我们尚未实施这些测试。

   首先，我们对测试的类上的每个公共方法使用单一测试方法进行开始，因此：

   | BylineImpl.java |  | BylineImplTest.java |
   | ------------------|--------------|---------------------|
   | getName() | 由 | testGetName() |
   | getSchrops() | 由 | testGetSchroplies() |
   | isEmpty() | 由 | testIsEmpty() |

   这些方法可以根据需要进行扩展，本章稍后将介绍。
...
1. 右键单击`BylineImplTest.java`文件，然后点按&#x200B;**运行**，运行JUnit测试用例。
...
在本教程中，使用了后一种方法（因为我们已在上一章中创建了一个有效的&#x200B;**BylineImpl.java**）。 因此，既要回顾和了解其公开方法的行为，又要了解其实施细节。 这听起来可能相反，因为良好的测试只应关注输入和输出，但在AEM工作时，需要了解各种执行考虑因素，以便构建工作测试。
...
由于单元测试是在内部版本执行的，因此在运行的AEM实例的上下文之外，不存在此类上下文。 为便于实现此目的，[wcm.io的AEM Mocks](https://wcm.io/testing/aem-mock/usage.html)创建了模拟上下文，使这些API能够像在AEM中运行一样运行到&#x200B;_大多_。
...
1. 在每个`@Test`方法之前执行的`setUp(..)`方法中，定义一个通用的模型测试状态：
   ...
   * **`load().json`** 将资源结构加载到模型上下文中，使代码能够与这些资源进行交互，就像它们是由真正的存储库提供的一样。文件&#x200B;**`BylineImplTest.json`**&#x200B;中的资源定义将加载到&#x200B;**/content**&#x200B;下的模型JCR上下文中。
   * **`BylineImplTest.json`** 尚不存在，因此，我们创建它并定义测试所需的JCR资源结构。

1. 表示模拟资源结构的JSON文件存储在&#x200B;**core/src/test/resources**&#x200B;下，其路径与JUnit Java测试文件相同。

   在名为&#x200B;**BylineImplTest.json**&#x200B;的&#x200B;**core/resources/com/adobe/aem/guides/wknd/core/models/impl**&#x200B;中新建一个名为的JSON文件，其内容如下：
   ...
   处理单元测试时的一般规则是创建满足每个测试所需的最少的模型内容、上下文和代码集。 避免在编写测试之前构建完整的模拟上下文的诱惑，因为这往往会导致不需要的文物。
...
1. 在`BylineImpl.java`中，如果`@PostConstruct init()`引发异常，则会阻止Sling模型实例化，并导致该Sling模型对象为null。
   ...
   * **`ctx.registerService(..)`。** 在AemContext中注册模型ModelFactory对象，其服务级别最高。这是必需的，因为BylineImpl`init()`中使用的ModelFactory是通过`@OSGiService ModelFactory model`字段注入的。 为了使AemContext插入&#x200B;**我们的**&#x200B;模型对象（处理对`getModelFromWrappedRequest(..)`的调用），我们必须将其注册为该类型(ModelFactory)的最高级别服务。
...
1. 重新运行测试，**`testGetName()`**&#x200B;现在通过！
   ...
   ![测试名称传递](assets/unit-testing/testgetname-pass.png)
...
![所有测试通过](./assets/unit-testing/all-tests-pass.png)
...
在[GitHub](https://github.com/adobe/aem-guides-wknd)上视图完成的代码，或在Git brach `tutorial/unit-testing-solution`上本地查看并部署代码。
