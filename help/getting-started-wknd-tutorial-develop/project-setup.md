...
为AEM创建Maven多模块项目有几个选项。 本教程将利用[Maven AEM Project Archetype **25**](https://github.com/adobe/aem-project-archetype)。 云管理器还[提供UI向导](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/getting-started/create-an-application-project.html)以启动AEM应用程序项目的创建。 云管理器UI生成的基础项目与直接使用原型的结构相同。

>[!NOTE]
>
>本教程使用原型的版本&#x200B;**25**。 使用&#x200B;**最新**&#x200B;版本的原型生成新项目始终是最佳做法。
...
1. 将以下内容粘贴到命令行中，以在批处理模式](https://maven.apache.org/archetype/maven-archetype-plugin/examples/generate-batch.html)中生成项目：[
   ...
   >[!NOTE]
   >
   > 如果使用AEM 6.5.5.0+或6.4.8.1+，请将`aemVersion="cloud"`替换为目标版AEM，即`aemVersion="6.5.5"`或`aemVersion="6.4.8.1"`
...
1. 已创建起始内容，并且可以向页面添加多个组件。 尝试这些组件，了解其功能。 您将在下一章中学习组件的基础知识。
...
生成的AEM项目由各个Maven模块组成，每个模块具有不同的角色。 本教程和大多数开发工作侧重于以下模块：
...
* [ui.content](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uicontent.html) -包含结构化内容和配置，如可编辑的模板、元数据模式(/content、/conf)。

* **all** -这是一个空的Maven模块，它将上述模块组合成可部署到AEM环境的单个包。

![Maven项目图](assets/project-setup/project-pom-structure.png)

请参阅[AEM Project Archetype文档](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/overview.html)，进一步了解&#x200B;**all** Maven模块。

### 包含核心组件{#core-components}

[AEM核心](https://docs.adobe.com/content/help/zh-Hans/experience-manager-core-components/using/introduction.html) 组件是一套针对AEM的标准化Web内容管理(WCM)组件。这些组件提供功能的基准集，设计为为各个项目设置样式、自定义和扩展。

AEM作为Cloud Service环境包括最新版本的[AEM核心组件](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/introduction.html)。 因此，作为Cloud Service为AEM生成的项目&#x200B;**不**&#x200B;包含嵌入的AEM核心组件。

对于AEM 6.5/6.4生成的项目，原型将自动在项目中嵌入[AEM核心组件](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/introduction.html)。 AEM 6.5/6.4嵌入AEM核心组件是确保将最新版本部署到项目的最佳实践。 有关如何将核心组件[包含在项目中的更多信息，请参阅](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/using.html#core-components)。
...
在`ui.apps`下方，观察已创建许多`.content.xml`文件。 这些XML文件映射JCR中安装的内容的节点类型和属性。 这些文件是关键文件，应&#x200B;**不应**&#x200B;被忽略。
...
## 高级Maven命令（奖励）{#advanced-maven-commands}
...
**[ui.apps](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uiapps.html)**&#x200B;主模块包含`/apps`下站点所需的所有渲染代码。 这包括将以AEM格式存储的CSS/JS，该格式名为[clientlibs](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/clientlibs.html)。 这还包括用于呈现动态HTML的[HTL](https://docs.adobe.com/content/help/zh-Hans/experience-manager-htl/using/overview.html)脚本。 您可以将&#x200B;**ui.apps**&#x200B;模块视为JCR中结构的映射，但格式可以存储在文件系统上并提交到源代码控制。 **ui.apps**&#x200B;模块只包含代码。
...
**[ui.content](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uicontent.htm)**&#x200B;模块的结构与&#x200B;**ui.apps**&#x200B;模块相同。 唯一的区别是&#x200B;**ui.content**&#x200B;模块包含所谓的&#x200B;**mutable**&#x200B;内容。 **多** 表集内容主要指非代码配置，如存储在源代码控制中的模板、策略或文件夹结构，但 **** 可以直接在AEM实例上修改。这将在页面和模板的一章中进行更详细的探讨。
