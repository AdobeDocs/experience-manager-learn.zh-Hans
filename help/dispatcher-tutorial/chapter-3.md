...
description: 这是AEM中缓存的三部分系列中的第3部分。 前两个部分重点介绍Dispatcher中的纯http缓存以及存在哪些限制。 本部分将讨论有关如何克服这些限制的一些想法。
feature: Dispatcher
topic: Architecture
role: Architect
level: Intermediate
...
这是AEM缓存系列三部分中的第3部分。 前两个部分重点介绍Dispatcher中的纯http缓存以及存在哪些限制。 本部分将讨论有关如何克服这些限制的一些想法。
...
在AEM世界中，当基础资源发生更改并将此更改事件传播到也可基于事件工作的Dispatcher时，Publish系统中的业务对象和HTML片段的内存缓存将失效。 例如，之前您将有一个基于TTL的CDN。
...
我们没有特别推广任何工具。 但是给你一些如何评估它们的提示。 例如，自6.0版起，AEM就有一个带有固定TTL的简单内置缓存。你会用吗？ 发布时可能不会，因为链中紧跟着基于事件的缓存(提示：Dispatcher)。 但对于作者来说，这或许是一个不错的选择。 AdobeACS Commons也有一个HTTP缓存，可能值得考虑。
...
例如，从负载平衡器中取出Publish/Dispatcher段以对其进行更新。 在重新集成它之前，您将自动爬网最常访问的页面，以将其重新放入缓存。 当缓存“热” — 已充分填充时，您将腿重新集成到负载平衡器中。
...
在实时系统上，访问模式并不是您在测试中拥有的少量均匀分布的页面（主页和少量内容页面）。 页数更大，请求分布非常不均匀。 并且 — 当然 — 实时页面无法从缓存获得100%的服务：来自Publish系统的失效请求使您宝贵100%的资源自动失效。

啊，是的。而且，当您重建Dispatcher缓存时，您会发现，Publish系统的行为也非常不同，这取决于您请求的是小部分页面还是更大的页面数量。 即使所有页面都同样复杂，它们的数量也会起到一定作用。 还记得我们对链式缓存的评价吗？ 如果总是请求同样数量的页面，则很可能包含原始数据的块位于硬盘缓存中，或者这些块由操作系统缓存。 此外，存储库也很可能在其主内存中缓存了相应的区段。 因此，重新渲染的速度比您之前让其他页面轮流从各种缓存中逐出要快得多。
...
现在，在Dispatcher中执行测试，确定完全不缓存。 这是你最坏的情况。 了解在这种最恶劣的情况下，您的系统会在哪个峰值负载变得不稳定。 如果你愿意，你也可以选择走几条Dispatcher/Publish的腿，让情况变得更糟。
...
你可能会回避这种努力。 但是，请至少对Publish系统进行最坏情况的测试，以查看其页数（平均分布）是否过大，以查看系统的限制。 确保，您能够正确解释最佳案例的数量，并为您的系统预留足够的空间。
