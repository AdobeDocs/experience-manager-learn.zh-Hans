...
description: 这是AEM中缓存的三部分系列中的第3部分。 前两个部分重点介绍Dispatcher中的纯http缓存以及存在哪些限制。 本部分讨论了一些如何克服这些限制的想法。
feature: Dispatcher
topic: Architecture
role: Developer
level: Intermediate
...
*“计算机科学中只有两个硬性内容：缓存失效和命名内容。”*

 — 菲尔·卡尔顿

## 概述

这是AEM缓存三部分系列中的第3部分。 前两个部分重点介绍Dispatcher中的纯http缓存以及存在哪些限制。 本部分讨论了一些如何克服这些限制的想法。

## 一般缓存

本系列的[第1](chapter-1.md)章和第[第2](chapter-2.md)章主要介绍Dispatcher。 我们已经说明了基本信息、限制以及您需要进行某些权衡的位置。

缓存的复杂性和复杂性不是Dispatcher特有的问题。 一般而言，缓存比较困难。

将Dispatcher作为工具箱中唯一工具实际上是一个真正的限制。

在本章中，我们要进一步扩大对缓存的看法，并提出一些如何克服Dispatcher的某些不足的想法。 没有灵丹妙药 — 您必须在项目中做出权衡。 请记住，在缓存和失效准确性中，总是伴随着复杂性，而在复杂性中，总是伴随着错误的可能性。
...
下一个级别是CRX或Oak - AEM使用的文档数据库。 CRX和Oak将数据划分为可在内存中缓存的区段，以避免访问硬盘速度变慢。
...
熄灭，将渲染的HTML-Page转至Dispatcher。 我们已经讨论过，Dispatcher的主要用途是缓存HTML页面和其他Web资源（尽管其名称为）。 在资源到达浏览器之前，它可能会传递一个反向代理（可以缓存）和一个也用于缓存的CDN。 客户端可能位于仅通过代理授予Web访问权限的办公室中，该代理可能决定缓存以节省流量。
...
**生存时间** — 如果对象的固有生存时间较短（流量数据的生存时间可能比天气数据短），则可能不适合缓存。

**生产成本 —**&#x200B;重新生产和交付对象的成本(就CPU周期和I/O而言)有多高。 如果它是廉价缓存，则可能没有必要。
...
上一章介绍的每个层都为缓存链提供了一些价值。 或者通过节省计算周期，或者通过让数据更接近消费者。 在链的多个阶段缓存一段数据并没有错，但您应该始终考虑下一阶段的好处和成本。 在Publish系统中缓存整页通常没有任何好处，因为在Dispatcher中已经这样做了。
...
* **过期日期：**&#x200B;对象在将来定义的时间过期（例如“2019年6月10日下午5:00”）
* **基于事件：**&#x200B;对象由平台中发生的事件显式失效（例如，当页面被更改和激活时）
...
始终使从内部到外部缓存失效。 如果先使外部缓存失效，则可能会重新缓存内部缓存的过时内容。 不要在缓存重新更新的时间进行任何假设 — 请确保。 最好是通过在&#x200B;_使内部缓存_&#x200B;失效后触发外部缓存的失效。
...
![基于TTL的未同步失效](assets/chapter-3/ttl-based-invalidation.png)
...
![延长内部对象生命周期的外部缓存](assets/chapter-3/outer-cache.png)
...
![正在同步到期日期](assets/chapter-3/synchronize-expiration-dates.png)
...
在AEM世界中，当基础资源发生更改并将此更改事件传播到也可基于事件工作的Dispatcher时，发布系统中会有业务对象和HTML片段的内存缓存，即失效。 例如，之前您将有一个基于TTL的CDN。

在Dispatcher前面设置一层（短）基于TTL的缓存可以有效地缓解自动失效后通常会出现的流量尖峰。
...
这里介绍的技术非常强大，每个AEM开发人员的工具箱中都有&#x200B;_必备项_。 但不要太兴奋，明智地使用它们。 通过将对象存储在缓存中并在后续请求中共享给其他用户，实际上意味着规避访问控制。 这在面向公众的网站上通常不是问题，但在用户需要登录才能访问网站时可能会成为问题。

考虑将站点主菜单的HTML标记存储在内存缓存中，以便在不同页面之间共享。 实际上，这是一个存储部分渲染的HTML的完美示例，因为创建导航通常很昂贵，因为它需要遍历大量页面。
...
4. 即使在AEM中的某个资源周围创建一个精简“包装器”，您也不得缓存该包装器，即使它是您自己的且不可变的包装器也是如此。 包装的对象将是一个引用（我们之前禁止使用该引用），如果我们看起来很锐利，则基本上会产生与上一项中所述相同的问题。

5. 如果要缓存，请通过将原始数据复制到自己的共享对象中来创建自己的对象。 您可能希望通过引用链接您自己的对象 — 例如，您可能希望缓存对象树。 没有问题 — 但只缓存您刚在同一请求中创建的对象 — 没有从其他位置请求的对象（即使是“您的”对象的名称空间也是如此）。 _正在复制对象_&#x200B;是键。 并确保立即清除链接对象的整个结构，并避免对结构的传入和传出引用。
...
我们没有特别推广任何工具。 但是给你一些如何评估它们的提示。 例如，自6.0版开始，AEM就有一个带有固定TTL的简单内置缓存。你会用吗？ 发布时可能不会，因为链中紧跟着基于事件的缓存(提示：Dispatcher)。 但对于作者来说，这或许是一个不错的选择。 Adobe ACS Commons也有一个HTTP缓存，可能值得考虑。

或者，您根据成熟的缓存框架（如[Ehcache](https://www.ehcache.org)）构建自己的缓存框架。 这可用于缓存Java对象和渲染的标记（`String`个对象）。
...
我们谈了很多无效和清除的事。 _缓存逐出_&#x200B;与这些术语相关：逐出某个条目后，该条目将再不可用。 但是，逐出不会发生在条目过期时，而是发生在缓存已满时。 较新项目或“较重要”项目将较旧项目或较不重要的项目推出缓存。 您需要牺牲哪些条目是逐一做出决定的。 您可能希望驱逐最旧的或很少使用或最后访问时间很长的那些文件。
...
例如，您从负载平衡器中取出“发布”/“Dispatcher”部分以对其进行更新。 在重新集成它之前，您将自动爬网最常访问的页面，以将其重新放入缓存。 当缓存“热” — 已充分填充时，您将腿重新集成到负载平衡器中。
...
![在不同页面上重新使用渲染的片段](assets/chapter-3/re-using-rendered-fragment.png)

*在不同页面上重新使用渲染的片段*

<br> 

HTML片段缓存是一个强大的工具。 其思想是将组件生成的HTML标记缓存在内存缓存中。 你可能会问，我为什么要那么做？ 我反正在Dispatcher中缓存整个页面的标记 — 包括该组件的标记。 我们同意。 您可以 — 但每页只能访问一次。 您未在页面之间共享该标记。

想象一下，您将在每个页面的顶部渲染导航。 每个页面上的标记看起来都一样。 但您正在对每个页面重复渲染它，Dispatcher中不存在这种情况。 并记住：自动失效后，所有页面都需要重新渲染。 基本上，你运行着相同的代码，其结果是数百次。
...
“密钥”是它在主页上具有的组件路径。 我们不在当前页面上使用组件的路径，因为这样会为每个页面创建一个缓存条目 — 这与我们共享该组件的意图相冲突。 我们也不只是使用组件相对路径(`jcr:conten/mainnavigation`)，因为这将阻止我们在不同站点中使用不同的导航组件。
...
在这种情况下，您可以给[Sling Dynamic Include](https://sling.apache.org/documentation/bundles/dynamic-includes.html)一次机会。 本质上，这是一个组件过滤器，它包装在动态组件周围，而不是将组件呈现到页面中，而是会创建引用。 此引用可以是Ajax调用 — 以便浏览器包含该组件，因此可以静态缓存周围的页面。 或者，Sling Dynamic Include可以生成SSI指令（服务器端包含）。 此指令将在Apache Server中执行。 如果您利用Varnish或支持ESI脚本的CDN，您甚至可以使用ESI - Edge Side Include指令。

使用Sling Dynamic Include的请求的![序列图](assets/chapter-3/sequence-diagram-sling-dynamic-include.png)

使用Sling Dynamic Include的请求的&#x200B;*序列图*

<br> 

SDI文档规定，在处理动态组件时，应禁用以“*.nocache.html”结尾的URL的缓存，这样才有意义。

您可能会看到另一个有关如何使用SDI的选项：如果您&#x200B;_不_&#x200B;为包含禁用Dispatcher缓存，则Dispatcher的行为类似于我们在上一章中描述的片段缓存：页面和组件片段均等地独立地缓存在Dispatcher中，并在请求页面时由Apache服务器中的SSI脚本拼合在一起。 这样，您就可以实施共享组件，如主导航（因为您始终使用相同的组件URL）。
...
但也许事实并非如此。 您可能希望为代表&#x200B;_当前页面_&#x200B;的导航中的项目呈现不同的标记。
...
这是两种完全不同的渲染。 但是，_业务对象_（完整的导航树）是相同的。  此处的&#x200B;_业务对象_&#x200B;将是一个表示树中节点的对象图。 此图形可以轻松地存储在内存缓存中。 但是，请记住，此图形不得包含任何对象或引用您未自行创建的任何对象，尤其是当前的JCR节点。
...
如果您正在优化Dispatcher设置以进行浏览器缓存，则在浏览器和Web服务器之间使用桌面代理服务器将非常有用。 我们更喜欢卡尔·冯·兰多的《Charles Web Debugging Proxy》。
...
“浏览器重新加载”、_简单重新加载_&#x200B;以及&#x200B;_强制重新加载_ （“_shift — 重新加载_”）与普通页面请求不同。 简单的重新加载请求会设置标头
...
因此，如果要调试正常浏览行为，您应该完全这样做：_正常浏览_。 使用浏览器的重新加载按钮是避免在配置中看到缓存配置错误的最佳方法。
...
一个常见的错误（我们经常看到）是，性能测试仅包含超少数量的页面，并且这些页面仅从Dispatcher缓存中交付。
...
是的，重建您的Dispatcher缓存时，您会发现，发布系统的行为也大不相同，这取决于您是只请求几页还是请求更多页面。 即使所有页面都同样复杂，它们的数量也会起到一定作用。 还记得我们对链式缓存的评价吗？ 如果总是请求同样数量的页面，则很可能包含原始数据的块位于硬盘缓存中，或者这些块由操作系统缓存。 此外，存储库也很可能在其主内存中缓存了相应的区段。 因此，重新渲染的速度比您之前让其他页面轮流从各种缓存中逐出要快得多。
...
现在，在Dispatcher中执行测试，确定完全不缓存。 这是你最坏的情况。 了解在这种最恶劣的情况下，您的系统会在哪个峰值负载变得不稳定。 如果您愿意，还可以通过走几条Dispatcher/Publish代码来雪上加霜。

接下来，执行相同的测试，将所有必需的缓存设置设为“开”。 慢慢提升并行请求以预热缓存，并了解在这些最佳情况下，您的系统可容纳的流量。

一种常见的情况是，在启用Dispatcher的情况下运行测试，但同时也会出现一些无效情况。 您可以通过cronjob接触statfile或不定时地向Dispatcher发送失效请求来模拟这种情况。 别忘了偶尔清除一些非自动失效的资源。
