...
如果HTTP响应符合上述限定符[的AEM Dispatcher缓存](#when-are-http-requestsresponses-cached)的条件，则除非存在自定义配置，否则以下是默认值。
...
[将CDN缓存内容的方式](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/content-delivery/caching.html?lang=zh-Hans#disp)限制为在HTTP响应中设置缓存标头。 这些缓存标头通常在使用`vhost`的AEM Dispatcher `mod_headers`配置中进行设置，但也可以在AEM Publish本身中运行的自定义Java™代码中进行设置。
...
+ 在Dispatcher配置[中明确配置](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=zh-Hans#configuring-time-based-cache-invalidation-enablettl)时的TTL （生存时间）。 通过查看[配置，查看](https://github.com/adobe/aem-project-archetype/blob/develop/src/main/archetype/dispatcher.cloud/src/conf.dispatcher.d/available_farms/default.farm#L122-L127)AEM项目原型`enableTTL`中的默认配置。

#### 默认缓存期限

如果HTTP响应符合上述限定符[的AEM Dispatcher缓存](#when-are-http-requestsresponses-cached-1)的条件，则除非存在自定义配置，否则以下是默认值。
