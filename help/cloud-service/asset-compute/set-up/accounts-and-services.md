...
description: 开发Asset compute工作程序需要访问帐户和服务，包括Microsoft或Amazon提供的AEMas a Cloud Service、App Builder和云存储。
...
>[!WARNING]
>
>在继续学习本教程之前，请确保您有权使用所有上述服务。
> 
> 请查看以下部分，了解如何设置并提供所需的服务。
...
1. 请等待大约2 - 10天，直到您通过电子邮件收到已配置的通知，然后再继续学习本教程。
   + 如果您不确定是否已配置，请继续后续步骤，如果您无法创建 __App Builder__ 中的项目 [Adobe Developer控制台](https://developer.adobe.com/console/) 您尚未配置。

## 云存储

本地开发Asset compute项目需要云存储。

当Asset compute工作程序部署到Adobe I/O Runtime以供AEMas a Cloud Service直接使用时，此云存储并不是绝对必需的，因为AEM提供了从中读取资产并向其写入演绎版的云存储。
...
1. 新建 __资源组__ 例如，根据需要： `aem-as-a-cloud-service`
...
使用 [Microsoft Azure Blob存储](#azure-blob-storage) 但是，建议完成本教程 [Amazon S3](https://aws.amazon.com/s3/?did=ft_card&amp;trk=ft_card) 也可以使用。
