...
了解如何集成AEM Assets和产品信息管理(PIM)系统以更新资源元数据 **使用AEM事件**. 在收到AEM Assets事件后，可以根据业务需求在AEM、PIM或两个系统中更新资源元数据。 但是，在此示例中，让我们更新AEM中的资源元数据。

运行资源元数据更新 **AEM外部的代码**， [Adobe I/O Runtime](https://developer.adobe.com/runtime/docs/guides/overview/what_is_runtime/) 使用无服务器平台。 事件处理流程如下所示：

![用于PIM集成的AEM Assets事件](../assets/examples/assets-pim-integration/aem-assets-pim-integration.png)

1. AEM Author服务触发 _资产处理已完成_ 资产上传完成时的事件。
...
1. Adobe I/O Runtime操作会调用模拟的PIM API以检索其他元数据，如SKU、供应商信息。
1. 检索到其他元数据的PIM随后在AEM Assets中使用 [资产创作API](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/experimental/assets/author/).
...
1. 在AEM创作服务中上传资源并验证元数据更新

有关1-2的详细步骤，请参阅 [Adobe I/O Runtime操作和AEM事件](./runtime-action.md#) 例如，对于3-6，请参阅以下部分。
...
- 然后选择 **AEM管理员 — XXX** 产品配置文件并单击 **保存配置的API**. 要访问粒度级功能和权限，必须将所选产品配置文件与生成AEMCS环境的AEM Assets事件关联。
...
- 此 `src/dx-excshell-1/actions/generic/index.js` file使用上述模块来编排元数据检索和更新。
...
- 资源元数据代码在AEM外部执行，避免了AEM Author服务上的负载，因此事件驱动的架构可以独立扩展。
...
- 通过AEM Eventing进行的Asset Events使企业能够自动化和简化关键流程，从而提高内容生态系统的效率和一致性。
