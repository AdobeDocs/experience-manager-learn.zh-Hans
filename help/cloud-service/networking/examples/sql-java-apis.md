...
description: 了解如何使用Java™ SQL API和出口端从AEMas a Cloud Service连接到SQL数据库。
...
与SQL数据库（和其他非HTTP/HTTPS服务）的连接必须从AEM中代理。

此规则的例外情况是 [专用出口ip地址](../dedicated-egress-ip-address.md) 正在使用，且服务在Adobe或Azure上。
...
由于密钥不能存储在代码中，因此最好通过 [密钥OSGi配置变量](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/configuring-osgi.html#secret-configuration-values)，使用AIO CLI或Cloud Manager API进行设置。
...
此Java™代码示例是OSGi服务的一个示例，该服务通过以下Cloud Manager连接到外部SQL Server Web服务器 `portForwards` 规则 [enableEnvironmentAdvancedNetworkingConfiguration](https://www.adobe.io/experience-cloud/cloud-manager/reference/api/#operation/enableEnvironmentAdvancedNetworkingConfiguration) 操作。
