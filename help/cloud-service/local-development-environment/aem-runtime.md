...
>title="本地AEM运行时"
>abstract="Adobe Experience Manager(AEM)可以使用AEM作为Cloud ServiceSDK的快速入门Jar在本地运行。 这允许开发人员在将自定义代码、配置和内容提交到源控件之前，先对其进行部署和测试，然后将其作为Cloud Service环境部署到AEM。"
...
Adobe Experience Manager(AEM)可以使用AEM作为Cloud ServiceSDK的快速入门Jar在本地运行。 这允许开发人员在将自定义代码、配置和内容提交到源控件之前，先对其进行部署和测试，然后将其作为Cloud Service环境部署到AEM。
...
## 下载AEM as aCloud ServiceSDK

AEM as a Dispatcher SDK或AEM SDK包含用于在本地运行AEM创作和发布以进行开发的快速入门Jar，以及兼容版本的调度程序工具。

1. 使用您的Adobe ID登录到[https://experience.adobe.com/#/downloads](https://experience.adobe.com/#/downloads)
   + 请注意，您的Adobe组织&#x200B;__必须__&#x200B;配置为AEM作为Cloud Service下载AEM as a Cloud ServiceSDK。
1. 导航到&#x200B;__AEM as a Cloud Service__&#x200B;选项卡
1. 按&#x200B;__发布日期__&#x200B;的顺序排序（按&#x200B;__降序__）
1. 单击最新的&#x200B;__AEM SDK__&#x200B;结果行
1. 查看并接受EULA，然后点按&#x200B;__Download__&#x200B;按钮
...
1. 将&#x200B;__快速入门JAR__&#x200B;文件复制到`~/aem-sdk/author`，然后将其重命名为`aem-author-p4502.jar`
1. 通过从命令行执行以下命令来启动本地AEM创作服务：
   + `java -jar aem-author-p4502.jar`
      + 将管理员密码提供为`admin`。 可接受任何管理员密码，但建议将默认密码用于本地开发，以减少重新配置的需要。

   通过双击](#troubleshooting-double-click),*不能*&#x200B;将AEM作为Cloud Service快速入门Jar [启动。
1. 在Web浏览器中，通过[http://localhost:4502](http://localhost:4502)访问本地AEM创作服务
...
本地AEM发布服务为开发人员提供AEM的本地体验最终用户将拥有的体验，例如浏览托管在AEM上的网站。 本地AEM发布服务非常重要，因为它与AEM SDK的[Dispatcher工具](./dispatcher-tools.md)集成，并允许开发人员进行烟雾测试和微调最终面向最终用户的体验。

1. 创建文件夹`~/aem-sdk/publish`
1. 将&#x200B;__快速入门JAR__&#x200B;文件复制到`~/aem-sdk/publish`，然后将其重命名为`aem-publish-p4503.jar`
...
1. 登录到&#x200B;**Author**&#x200B;服务，然后导航到[http://localhost:4502/etc/replication/agents.author.html](http://localhost:4502/etc/replication/agents.author.html)。
1. 单击&#x200B;**默认代理（发布）**&#x200B;以打开默认复制代理。
1. 单击&#x200B;**编辑**&#x200B;以打开代理的配置。
1. 在&#x200B;**Settings**&#x200B;选项卡下，更新以下字段：
...
快速入门Jar的命名`aem-<tier>_<environment>-p<port number>.jar`指定其启动方式。 在特定层、创作或发布中启动AEM后，便无法将其更改为替代层。 为此，必须删除在首次运行期间生成的`crx-Quickstart`文件夹，并且必须再次运行快速入门Jar。 可以更改环境和端口，但需要停止/启动本地AEM实例。

更改环境（`dev`、`stage`和`prod`）对于开发人员而言非常有用，可确保AEM正确定义和解析特定于环境的配置。 建议主要针对默认的`dev`环境运行模式进行本地开发。
...
+ `aem-author_dev-p4502.jar`
   + 作为在端口4502上的开发运行模式下的作者（与`aem-author-p4502.jar`相同）
...
+ `aem-publish_dev-p4503.jar`
   + 作为在端口4503的开发运行模式下的作者（与`aem-publish-p4503.jar`相同）
...
+ __OSGi配置环境变量和密钥变__ 量是特 [别为AEM本地运行时设置的](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/configuring-osgi.html#local-development)，而不是使用aio CLI管理它们。

## 何时更新快速入门Jar

每月最后一个星期四(即AEM作为Cloud Service“功能发布”的发行频率)，即至少每月或之后不久更新AEM SDK。
...
这是因为AEM as a Cloud Service快速入门Jar不支持双击快速入门Jar以在本地启动AEM。 而是必须从该命令行运行Jar文件。

要启动AEM创作服务，请将`cd`放入包含快速入门Jar的目录中，然后执行命令：

`$ java -jar aem-author-p4502.jar`

或者，要启动AEM发布服务，请将`cd`放入包含快速入门Jar的目录中，然后执行命令：
...
这是因为AEM as a Cloud Service需要Java SDK 11，并且您运行的是其他版本，很可能是Java 8。 要解决此问题，请下载并安装[OracleJava SDK 11](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?1_group.propertyvalues.property=.%2Fjcr%3Acontent%2Fmetadata%2Fdc%3AsoftwareType&amp;1_group.propertyvalues.operation=equals&amp;1_group.propertyvalues.0_values=software-type%3Atoling&amp;fulltext=Oracle%7E+JDK%7E+11%7E&amp;orderby=%40jcr%3Cont%2Fjcr%3Alast&amp;by.sort=desc&amp;list=0&amp;p.offset=14)。
