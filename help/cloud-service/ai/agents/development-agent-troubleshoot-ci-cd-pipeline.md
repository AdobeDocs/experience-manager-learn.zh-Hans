...
- 为WKND Sites项目创建新的Cloud Manager Git存储库，并将其作为远程存储库添加到本地Git存储库：
   ...
   - 输入&#x200B;**存储库名称**（例如，“wknd-site-tutorial”），然后单击&#x200B;**保存**。 等待创建存储库。
     ...
     ![添加存储库](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-repository.png)

   - 单击右上角的&#x200B;**访问存储库信息**&#x200B;并复制存储库URL。

     ![访问存储库信息](./assets/development-agent-troubleshoot-ci-cd-pipeline/access-repo-info.png)
...
- 在&#x200B;**添加非生产管道**&#x200B;对话框中，配置以下内容：
   ...
   - **配置**&#x200B;步骤：
     ...
     ![添加非生产管道配置](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-configuration.png)
   ...
   - **Source代码**&#x200B;步骤：
     ...
     ![添加非生产管道Source代码](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-source-code.png)

- 在管道条目的三点式菜单中单击&#x200B;**运行**，运行新创建的代码质量管道。

  ![运行代码质量管道](./assets/development-agent-troubleshoot-ci-cd-pipeline/run-code-quality-pipeline.png)
...
![失败的管道执行](./assets/development-agent-troubleshoot-ci-cd-pipeline/failed-pipeline-execution.png)
...
- 以自然语言（即&#x200B;**提示**）输入管道失败详细信息。 例如：
  ...
  ![调用AEM开发代理](./assets/development-agent-troubleshoot-ci-cd-pipeline/invoke-aem-development-agent.png)
...
- 推理完成后，单击&#x200B;**全屏打开**&#x200B;图标以查看详细的故障排除过程。
  ...
  ![全屏打开](./assets/development-agent-troubleshoot-ci-cd-pipeline/open-in-full-screen.png)
...
- 在这种情况下，代理正确地建议更改实现（`getName()`方法）或更新单元测试（`getNameTest()`方法）以解决此问题。 它避免了幻觉，并采用了human-in-the-loop方法，同时为开发人员提供了可操作的代码更改。
  ...
  ![复制代码更改](./assets/development-agent-troubleshoot-ci-cd-pipeline/copy-code-changes.png)
...
![比较分支](./assets/development-agent-troubleshoot-ci-cd-pipeline/compare-branches.png)
