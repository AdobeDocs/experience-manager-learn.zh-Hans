...
![Cloud Manage构建管道](./assets/build-and-deployment/build-pipeline.png)
...
构建和单元测试阶段执行Maven构建(`mvn clean package`)，该项目是从管道配置的Git分支中签出的。

在此阶段发现的错误应该可以在本地重新生成项目，但以下情况除外：

+ Maven依赖项在以下位置不可用： [Maven Central](https://search.maven.org/) 将使用，并且包含依赖项的Maven存储库满足以下任一条件：
...
+ __原因：__ AEM项目的all包，包含多个代码包，并且多个代码包提供了相同的OSGi配置，这会导致冲突，导致“生成图像”步骤无法决定应该使用哪个代码包，从而导致“生成”失败。 请注意，这不适用于OSGi工厂配置，只要它们的名称是唯一的。
+ __分辨率：__ 审查作为AEM应用程序的一部分部署的所有代码包（包括任何包含的第三方代码包），查找通过运行模式解析到目标环境的重复OSGi配置。 在AEM as a Cloud Service中不可能使用错误消息的“将mergeConfigurations标志设置为true”指南，应当忽略该指南。

#### 原因2

+ __原因：__ AEM项目错误地两次包含相同的代码包，从而导致所述包中包含的任何OSGi配置重复。
...
+ 部署应用程序会更新中的核心组件maven依赖关系版本 `core` （OSGi捆绑包）项目
+ 然后，将部署应用程序部署到沙盒（非生产）AEMas a Cloud Service环境，该环境尚未更新为使用包含该新核心组件版本的AEM版本。

为防止出现此故障，只要有AEMas a Cloud Service环境的更新可用，就应在下次生成/部署中包括更新，并始终确保在应用程序代码库中增加核心组件版本之后包括更新。

+ __症状：__
“构建映像”步骤失败，并出现“错误”报告 `com.adobe.cq.wcm.core.components...` 特定版本范围内的包无法由导入 `core` 项目。
...
熟悉 [AEMas a Cloud Service日志](./logs.md) 在调试“部署到”步骤之前。 此 `aemerror` 日志包含有关pod启动和关闭的信息，这可能与“部署到问题”相关。 请注意，通过Cloud Manager的部署到步骤中的下载日志按钮提供的日志不是 `aemerror` 日志，但不包含与您的应用程序启动相关的详细信息。
...
+ __原因：__ 自定义代码可以执行在OSGi捆绑包或组件生命周期早期触发的操作，例如大型查询或内容遍历，这会显着延迟AEM的启动时间。
+ __分辨率：__ 查看在OSGi捆绑包的生命周期早期运行的代码实施，并查看 `aemerror` Cloud Manager所显示的故障时间（以GMT为单位记录时间）前后的AEM创作和发布服务的日志，并查找指示任何自定义日志运行进程的日志消息。

### 不兼容的代码或配置

大多数代码和配置违规是在内部版本中的早期捕获的，但是，自定义代码或配置可能与AEMas a Cloud Service不兼容，并且在容器中执行之前不会检测到。

+ __原因：__ 自定义代码可能会调用很长的操作，例如在OSGi捆绑包或组件生命周期早期触发的大型查询或内容遍历，这会显着延迟AEM的启动时间。
+ __分辨率：__ 查看 `aemerror` Cloud Manager显示的故障发生时间（以GMT为单位记录时间）前后的AEM创作和发布服务的日志。
...
`/var` 是可变的，包含各种临时运行时内容。 包括 `/var` 在内容包中(例如， `ui.content`)通过Cloud Manager部署可能会导致部署步骤失败。
...
+ __原因：__ 用于将内容包部署到AEM Publish服务的AEM复制服务用户无法写入 `/var` 在AEM Publish上。 这会导致将内容包部署到AEM Publish服务失败。
+ __分辨率：__ 以下解决此问题的方法按优先顺序列出：
   1. 如果 `/var` 资源无需删除下的任何资源 `/var` 通过作为应用程序的一部分部署的内容包。
   2. 如果 `/var` 资源是必需的，请使用定义节点结构 [repoinit](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit). Repoinit脚本可以通过OSGi运行模式定位到AEM Author和/或AEM Publish。
   3. 如果 `/var` 资源仅在AEM创作上需要，不能使用进行合理建模 [repoinit](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit)，将它们移至离散的内容包，该内容包仅由安装在AEM Author上 [嵌入](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) 它位于 `all` 在AEM Author运行模式文件夹中包(`<target>/apps/example-packages/content/install.author</target>`)。
   4. 为提供适当的ACL `sling-distribution-importer` 服务用户，如本中所述 [Adobe知识库](https://helpx.adobe.com/in/experience-manager/kb/cm/cloudmanager-deploy-fails-due-to-sling-distribution-aem.html).
