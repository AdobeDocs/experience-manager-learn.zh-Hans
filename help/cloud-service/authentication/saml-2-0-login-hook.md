...
+ 已配置具有适当权限的[服务用户](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-learn/cloud-service/developing/advanced/service-users)（在“Apache Sling服务用户映射器服务修正”中配置）
...
如果SAML挂接需要修改AEM JCR存储库中的内容，例如用户属性（如`postSyncUserProcess`示例中所示），则必须配置[服务用户](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-learn/cloud-service/developing/advanced/service-users)：
...
+ **服务用户权限**：向[服务用户](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-learn/cloud-service/developing/advanced/service-users)授予所需的最低权限（例如，仅`jcr:read`上的`rep:write`和`/home/users`，不是完全的管理权限）
