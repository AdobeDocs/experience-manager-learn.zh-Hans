...
需要添加`package-info.java`，以确保AEM中的其他OSGi捆绑包能够解析OSGi服务接口（或任何Java类）。 如果`package-info.java`缺失，则不会导出Java包及其Java接口或类。 尝试从此Java包导入这些Java接口或类的其他OSGi包将出错，并出现消息&#x200B;__无法解析__ AEM的OSGi包控制台。
