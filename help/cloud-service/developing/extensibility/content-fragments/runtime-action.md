---
title: AEM内容片段控制台扩展Adobe I/O Runtime操作
...
![AEM内容片段扩展运行时操作](./assets/runtime-action/action-runtime-flow.png){align="center"}

AEM内容片段扩展可以选择包含任意数量的 [Adobe I/O Runtime操作](https://developer.adobe.com/runtime/docs/).

Adobe I/O Runtime操作是可由扩展调用的无服务器函数。 操作对于执行需要与AEM或其他AdobeWeb服务交互的工作非常有用。
操作通常对执行长时间运行（任何时间超过几秒钟）的任务或对AEM或其他Web服务发出HTTP请求最有用。

使用Adobe I/O Runtime操作执行工作的好处包括：

+ 操作是在浏览器上下文之外运行的无服务器函数，无需担心CORS
+ 用户无法中断操作（例如，刷新浏览器）
+ 操作是异步执行的，因此它们可以根据需要长时间运行，而不会阻止用户

在AEM内容片段扩展的上下文中，操作通常用于直接与AEMas a Cloud Service通信，通常：

+ 从AEM收集有关内容片段的相关数据
+ 对内容片段执行自定义操作
+ 定制内容片段创建

当AEM内容片段扩展在内容片段控制台中显示时，扩展及其支持操作可以调用任何可用的AEM HTTP API，包括自定义AEM API端点。

## 调用操作

Adobe I/O Runtime操作主要从AEM内容片段扩展中的两个位置调用：

1. 的 [扩展注册](./extension-registration.md) `onClick(..)` 处理程序
1. 在 [模态](./modal.md)
...
可以直接从模式调用Adobe I/O Runtime操作以执行更多涉及的工作，特别是依赖于与AEMas a Cloud Service、Adobeweb服务，甚至第三方服务通信的工作。

Adobe I/O Runtime操作是在无服务器Adobe I/O Runtime环境中运行的基于Node.js的JavaScript应用程序。 这些操作可通过扩展SPA通过HTTP进行寻址。
