...
description: 创建可与AdobeIMS API交换的JSON Web令牌(JWT)，以获取AEM访问令牌。
type: Documentation
role: Developer
...
JSON Web令牌是一种开放的行业标准RFC 7519方法，用于在双方之间安全地表示声明。 此示例中使用了JWT.io库来生成JWT。
您在上一步中下载的服务凭据包含PKCS#1格式中的私钥。若要从此字符串中提取私钥，我们使用了[BouncyCastle](https://www.bouncycastle.org/)库。 Java中的加密库不支持PKCS#1格式。
