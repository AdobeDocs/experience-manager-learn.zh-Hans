---
title: AEM内容片段的翻译支持
description: 了解如何使用Adobe Experience Manager对内容片段进行本地化和翻译。 还有资格提取和翻译与内容片段关联的混合媒体资产。
...
了解如何使用Adobe Experience Manager对内容片段进行本地化和翻译。 还有资格提取和翻译与内容片段关联的混合媒体资产。
...
2. 为语言副本选择站点页面时，在站点页面上引用的内容片段将复制到相应的语言文件夹并提取以供翻译。
3. 嵌入内容片段中的内联媒体资产有资格进行提取和翻译。
...
开箱即用的配置引用内容片段，位于 `fragmentPath` 资源类型为 `core/wcm/components/contentfragment/v1/contentfragment`. 继承自以下项的所有组件： `v1/contentfragment` 默认配置可识别。
