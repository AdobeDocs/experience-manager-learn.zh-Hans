...
在本教程中，让我们探索通过更新WKND来扩展Adobe客户端数据层的各种选项 [署名组件](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-wknd-tutorial-develop/project-archetype/custom-component.html). 的 _署名_ 组件是 **自定义组件** 本教程中学习的和课程可以应用到其他自定义组件。
...
1. 将WKND代码库部署到AEM的本地实例：
   ...
   >[!NOTE]
   >
   > 对于AEM 6.5和最新的Service Pack，请添加 `classic` Maven命令的配置文件：
   >
...
要在数据层中插入有关组件的数据，我们先更新组件的Sling模型。 接下来，更新Byline的Java™界面和Sling Model实施，以使用新方法 `getData()`. 此方法包含要注入到数据层中的属性。
...
1. 打开文件 `BylineImpl.java` at `core/src/main/java/com/adobe/aem/guides/wknd/core/models/impl/BylineImpl.java`. 它是 `Byline` 接口，作为Sling模型实施。
...
1. 将以下import语句添加到文件的开头：
   ...
   的 `fasterxml.jackson` API用于序列化要以JSON形式公开的数据。 的 `ComponentUtils` 的AEM核心组件来检查数据层是否已启用。
...
1. 添加未实现的方法 `getData()` to `BylineImple.java`:
   ...
   在上述方法中，新 `HashMap` 用于捕获要以JSON形式显示的属性。 请注意，现有方法如 `getName()` 和 `getOccupations()` 中，将使用。 的 `@type` 表示组件的唯一资源类型，它允许客户端根据组件类型轻松识别事件和/或触发器。
...
1. 返回到浏览器并重新打开添加了署名组件的页面： [http://localhost:4502/content/wknd/us/en/magazine/guide-la-skateparks.html](http://localhost:4502/content/wknd/us/en/magazine/guide-la-skateparks.html).
...
1. 在浏览器中，使用 **控制台** 打开，在署名组件中单击作者的姓名：
   ...
   的 `cmp:click` 事件是最容易挂接的事件。 对于更复杂的组件和要跟踪其他行为，可以添加自定义JavaScript以添加和注册新事件。 轮播组件就是一个很好的示例，它会触发 `cmp:show` 切换幻灯片时发生的事件。 请参阅 [更多详细信息的源代码](https://github.com/adobe/aem-core-wcm-components/blob/main/content/src/content/jcr_root/apps/core/wcm/components/carousel/v1/carousel/clientlibs/site/js/carousel.js).
...
1. 替换 `getData()` 方法：
   ...
   署名组件会重新使用图像核心组件的部分，以显示代表作者的图像。 在上述代码片段中， [数据层生成器](https://javadoc.io/doc/com.adobe.cq/core.wcm.components.core/latest/com/adobe/cq/wcm/core/components/models/datalayer/builder/ComponentDataBuilder.html) 用于扩展 `Image` 组件。 这会使用有关所用图像的所有数据预填充JSON对象。 它还会执行一些例程功能，例如设置 `@type` 和组件的唯一标识符。 请注意，方法很小！
...
1. 通过检查 `ImageList` 组件：
   ...
   * `image-list.html` - HTL模板(位于 `ui.apps` 模块。
   ...
   >[!NOTE]
   >
   > 如果为整个实施中重复使用的对象构建一个高级数据层，则建议将数据层元素提取到其自己特定于数据层的Java™对象中。 例如，商务核心组件为 `ProductData` 和 `CategoryData` 因为这些组件可用于商务实施中的许多组件。 审阅 [aem-cif-core-components存储库中的代码](https://github.com/adobe/aem-core-cif-components/tree/master/bundles/core/src/main/java/com/adobe/cq/commerce/core/components/datalayer) 以了解更多详细信息。
