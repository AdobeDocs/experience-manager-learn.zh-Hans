...
# 了解如何为样式系统编写代码{#understanding-how-to-code-for-the-aem-style-system}

在此视频中，我们将了解使用样式系统设计Experience Manage的核心标题组件样式的CSS(或 [!DNL LESS])和JavaScript的剖析，以及这些样式如何应用于HTML和DOM。

>[!NOTE]
>
>AEM Style System随AEM [6.3 SP1](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp1-release-notes.html) + Feature Pack [20593而推出](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/featurepack/cq-6.3.0-featurepack-20593)。
>
>视频假定We.Retail Title组件已更新为继承 [核心组件v2.0.0+](https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/releases)。

## 了解如何为样式系统编写代码 {#understanding-how-to-code-for-the-style-system}

>[!VIDEO](https://video.tv.adobe.com/v/21538/?quality=9&learn=on)

提供的AEM包(**technical-review.sites.style-system-1.0.0.zip**)将安装示例标题样式、We.Retail布局容器和标题组件的示例策略以及示例页面。

[technical-review.sites.style-system-1.0.0.zip](assets/technical-review.sites.style-system-1.0.0.zip)

### CSS {#the-css}

以下是示例 [!DNL LESS] 样式的定义，可在找到：

* `/apps/demo/sites/style-system/clientlib-example/components/titles/styles/example.less`

对于喜欢CSS的人，此代码片断下面是编译为的 [!DNL LESS] CSS。
...
以上内 [!DNL LESS] 容由Experience Manager到以下CSS进行本机编译。
...
>[!NOTE]
>
>样式系统与边界元法 [有很小的](https://en.bem.info/)差异，即 `BLOCK` 和不 `BLOCK--MODIFIER` 应用于边界元法所指 [定的同一](https://en.bem.info/)元素。
>
>而是由于产品约束， `BLOCK--MODIFIER` 将应用于元素的父 `BLOCK` 项。
>
>边界元的所 [有其](https://en.bem.info/) 他租户都应与之保持一致。

* 使用LESS [(受AEM本](https://lesscss.org/) 机支持 [](https://sass-lang.com/) )或SCSS（需要自定义构建系统）等预处理器，可以清除CSS定义并重新实现可用性。

* 保持选择器权重/特异性一致；这有助于避免和解决难以识别的CSS层叠冲突。
* 将每种样式组织到一个离散文件中。
   * 这些文件可以使用LESS/SCSS `@imports` 组合，如果需要原始CSS，也可以通过HTML客户端库文件包含或自定义前端资源构建系统组合。
* 避免混用许多复杂的样式。
...
* 在将其附 `BLOCK--MODIFIER` 加到响应式网格时，避免直接设置样式。 更改此元素的显示可能会影响响应式网格的呈现和功能，因此当更改响应式网格的行为时，只有此级别的样式才会受到影响。
* 使用应用样式范围 `BLOCK--MODIFIER`。 可 `BLOCK__ELEMENT--MODIFIERS` 以在组件中使用，但由于 `BLOCK` 组件表示组件，并且组件是样式，样式是“定义”的，其作用域为 `BLOCK--MODIFIER`。
...
对于嵌套组件，这些嵌套组件元素的CSS选择器深度将超过第3级选择器。 为嵌套组件重复相同的模式，但范围由父组件的父组件 `BLOCK`。 换言之，将嵌套组件开始 `BLOCK` 到第3级，而嵌套组件 `ELEMENT` 到第4个选择器级。
...
* 处理发布和创作体验。
* 尽可能重用样式Javascript。
   * 例如，如果某个组件的多个样式要求将其图像移动到背景图像，则可以实现style-JavaScript并将其附加到多个 `BLOCK--MODIFIERs`。
...
* 要避免闪烁和不必要的重绘，请首先通过隐藏组件， `BLOCK--MODIFIER BLOCK`并在JavaScript中的所有DOM操作完成时显示它。
