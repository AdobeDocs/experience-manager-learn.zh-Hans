...
>[!NOTE]
>
>请阅读[了解如何为样式系统编码](style-system-technical-video-understand.md)中的内容，以确保理解AEM样式系统使用的类似BEM的惯例。
...
**布局** 样式会影响组件的许多元素，以创建组件的良好定义和可识别的再现（设计和布局），通常与特定的可重用品牌概念保持一致。例如，Teaser组件可以采用传统的基于卡的布局、水平促销样式或图像上覆盖文本的Hero布局。

**显** 示样式用于影响布局样式的细微变化，但它们不会更改布局样式的基本性质或用途。例如，Hero布局样式可能具有将颜色方案从主品牌颜色方案更改为辅助品牌颜色方案的显示样式。

## 设计组织最佳实践样式{#style-organization-best-practices}
...
组件必须具有用品牌的&#x200B;**主**&#x200B;和&#x200B;**辅助**&#x200B;颜色着色的选项，但AEM作者知道颜色为&#x200B;**绿色**&#x200B;和&#x200B;**黄色**，而不是主和辅助的设计语言。
...
## 作为示例的Teaser组件用例{#the-teaser-component-as-an-example-use-case}
...
下图显示了Teaser组件的[!UICONTROL Styles]配置，以了解用例中讨论的变量。

[!UICONTROL 样式组]名称、布局和显示（偶发情况）与本文中用于概念分类样式的显示样式和布局样式的一般概念相匹配。
...
下图显示作者与[!UICONTROL Style]菜单作者交互以选择组件的相应样式。 请注意，[!UICONTROL 样式Grpi]名称以及样式名称均向作者公开。
...
如果默认样式规则应用于所有变量的频率更高，最好使用`.cmp-teaser`作为默认样式的CSS类，因为所有变量都应隐式继承它们，假定遵循类似BEM的惯例。 否则，应通过默认修饰符（如`.cmp-teaser--default`）应用这些样式，后者又需要添加到[组件的样式配置的“默认CSS类”](#component-styles-configuration)字段中，否则，这些样式规则必须在每个变体中被覆盖。

甚至可以将“named”样式指定为默认样式，例如，下面定义的Hero样式`(.cmp-teaser--hero)`，但更清楚的是，要针对`.cmp-teaser`或`.cmp-teaser--default` CSS类实现实现默认样式。
...
为此，在Teaser组件的AEM样式系统中配置了&#x200B;**Promo**&#x200B;的&#x200B;**布局样式**&#x200B;和&#x200B;**Green**&#x200B;和&#x200B;**Yellow**&#x200B;的&#x200B;**显示样式**。
...
**促销右对齐**&#x200B;布局样式是促销样式的变体，该样式会翻转图像和文本的位置（右图，左图文本）。
...
请注意，CSS类的名称`cmp-teaser--alternate`不必与“right aligned”的对作者友好的命名法相匹配。
...
用主（绿色）颜色为&#x200B;**Promote**&#x200B;布局样式的背景着色的CSS:
