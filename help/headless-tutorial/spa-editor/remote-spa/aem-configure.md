...
description: 需要使用AEM项目来设置支持配置和内容要求，以允许AEM SPA编辑器创作远程SPA。
...
创建一个AEM项目，其中管理配置和基线内容。 此项目将在克隆的`aem-guides-wknd-graphql`项目的`remote-spa-tutorial`文件夹中生成。
...
在指定`frontendModule="react"`时，`ui.frontend`项目未用于远程SPA用例。 SPA是在外部开发并管理到AEM的，并且仅将AEM用作内容API。 项目需要`frontendModule="react"`标记才能包含`spa-project`个AEM Java™依赖项并设置远程SPA页面模板。
...
由于SPA是远程SPA，因此假定它是在AEM项目之外开发和管理的。 为避免冲突，请从部署中删除`ui.frontend`项目。 如果未删除`ui.frontend`项目，则会在AEM SPA编辑器中同时加载两个SPA（`ui.frontend`项目和远程SPA中提供的默认SPA）。
...
接下来，配置AEM以保护内容，以便仅此SPA可以访问AEM内容。 在AEM[&#128279;](https://experienceleague.adobe.com/docs/experience-manager-learn/foundation/security/develop-for-cross-origin-resource-sharing.html?lang=zh-Hans)中配置跨源资源共享。
...
在部署AEM项目后，最后一个步骤是准备SPA编辑器以加载我们的远程SPA。 在AEM中，标记与SPA的根`/content/wknd-app/us/en/home`对应的AEM页面，该根由AEM项目原型生成。

1. 登录AEM Author
1. 导航到&#x200B;**站点> WKND应用程序>我们> en**
1. 选择&#x200B;**WKND应用程序主页**，然后点按&#x200B;**属性**

   ![WKND应用程序主页 — 属性](./assets/aem-content/edit-home-properties.png)

1. 导航到&#x200B;**SPA**&#x200B;选项卡
1. 填写&#x200B;**远程SPA配置**
   1. **SPA主机URL**： `http://localhost:3000`
      1. 到远程SPA根目录的URL
...
配置AEM后，我们可以重点使用[引导远程SPA](./spa-bootstrap.md)，并使用AEM SPA Editor支持可编辑区域！
