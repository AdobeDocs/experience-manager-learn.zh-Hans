...
description: 需要一个AEM项目来设置支持配置和内容要求，以允许AEM SPA Editor创作远程SPA。
...
虽然SPA代码库是在AEM之外管理的，但需要一个AEM项目来设置支持配置和内容要求。 本章介绍如何创建包含必要配置的AEM项目：
...
While `frontendModule="react"` 指定，则 `ui.frontend` 项目不用于远程SPA用例。 SPA是在AEM外部开发和管理的，并且仅将AEM用作内容API。 此 `frontendModule="react"` 项目需要标志包括  `spa-project` AEM Java™依赖项并设置远程SPA页面模板。
...
由于SPA是远程SPA，因此假定它是在AEM项目之外开发和管理的。 为避免冲突，请删除 `ui.frontend` 部署中的项目。 如果 `ui.frontend` 未删除项目，两个SPA，即中提供的默认SPA `ui.frontend` 项目与远程SPA同时加载到AEM SPA编辑器中。
...
映射可以使用 [Sling映射](https://sling.apache.org/documentation/the-sling-engine/mappings-for-resource-resolution.html#root-level-mappings-1) 在中定义 `/etc/map`.
...
1. In `etc`，创建文件夹 `map`
1. In `map`，创建文件夹 `http`
1. In `http`，创建文件 `.content.xml` ，其内容为：
...
1. In `http` ，创建文件夹 `localhost_any`
1. In `localhost_any`，创建文件 `.content.xml` ，其内容为：
...
1. In `localhost_any` ，创建文件夹 `wknd-app-routes-adventure`
1. In `wknd-app-routes-adventure`，创建文件 `.content.xml` ，其内容为：
...
运行AEM的Sling映射效果 `http` 和 `localhost`，因此仅支持本地开发。 部署到AEMas a Cloud Service时，必须将类似的Sling映射添加到该目标 `https` 以及相应的AEMas a Cloud Service域。欲了解更多信息，请参见 [Sling映射文档](https://sling.apache.org/documentation/the-sling-engine/mappings-for-resource-resolution.html).

## 跨源资源共享安全策略

接下来，配置AEM以保护内容，以便只有此SPA才能访问AEM内容。 配置 [AEM中的跨源资源共享](https://experienceleague.adobe.com/docs/experience-manager-learn/foundation/security/develop-for-cross-origin-resource-sharing.html).
...
+ `alloworigin` 指定允许哪些主机从AEM检索内容。
   ...
   + `https://external-hosted-app` 充当占位符，以替换为Remote SPA托管所在的域。
+ `allowedpaths` 指定此CORS配置涵盖AEM中的哪些路径。 默认允许访问AEM中的所有内容，但可以仅将其范围限定为SPA可以访问的特定路径，例如： `/content/wknd-app`.

## 将AEM页设置为远程SPA页模板

AEM项目原型会生成一个项目，该项目已准备好与远程SPA进行AEM集成，但需要对自动生成的AEM页面结构进行小幅但重要的调整。 自动生成的AEM页面的类型必须更改为 __“远程SPA”页__，而不是 __SPA页面__.
...
>[!NOTE]
>
>如果此项目之前已部署到AEM，请确保删除AEM页面，如下所示 __站点> WKND应用程序>我们> en > WKND应用程序主页__，作为 `ui.content`  项目设置为 __merge__ 节点，而不是 __更新__.

不过，也可以删除此页面，并将其重新创建为AEM中的远程SPA页面，因为该页面是在 `ui.content` 在代码库中对其进行更新是最佳选择。

## 将AEM项目部署到AEM SDK

1. 确保AEM创作服务在端口4502上运行
1. 在命令行中，导航到AEM Maven项目的根目录
...
部署AEM项目后，最后一个步骤是准备SPA编辑器以加载我们的远程SPA。 在AEM中，标记与SPA根目录对应的AEM页面，`/content/wknd-app/us/en/home`，由AEM项目原型生成。
...
请记住，我们更改了此页面的类型 __“远程SPA”页__，这让我们能够查看 __SPA__ 选项卡中的 __页面属性__.

只能在与AEM根目录相对应的SPA页面上设置此配置。 此页面下的所有AEM页面都会继承该值。
...
+ 设置AEM跨源资源共享安全策略，以允许远程SPA使用来自AEM的内容
+ 将AEM项目部署到本地AEM SDK作者服务
+ 使用AEM主机URL页属性将SPA页标记为远程SPA根

## 后续步骤

在配置AEM后，我们可以 [引导远程SPA](./spa-bootstrap.md) 支持使用AEM SPA编辑器编辑区域！
