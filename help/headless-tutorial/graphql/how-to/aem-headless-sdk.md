...
使用GraphQL通过持久查询(与[客户端定义的AEM查询](#graphl-queries)不同)来查询AEM允许开发人员在GraphQL中保留查询（但不保留其结果），然后请求按名称执行查询。 持久查询与SQL数据库中存储过程的概念类似。

持久查询比客户端定义的GraphQL查询性能更高，因为持久查询使用HTTP GET(可在CDN和AEM Dispatcher层缓存)执行。 持久查询也有效，定义了API，并降低了开发人员了解每个内容片段模型详细信息的需要。
...
此代码示例说明如何使用[React useEffect(..)挂接](https://reactjs.org/docs/hooks-effect.html)执行对AEM GraphQL的异步调用。
...
AEM Headless JS SDK对`util`具有依赖关系，默认情况下，该依赖关系不在Webpack 5+中。 如果您使用的是Webpack 5+，则会收到以下错误：
