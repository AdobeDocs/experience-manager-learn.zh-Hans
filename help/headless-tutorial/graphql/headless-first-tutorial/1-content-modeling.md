...
内容片段提供了一种结构化的方法来跨渠道管理内容，提供了灵活性和可重用性。 在AEM中启用内容片段允许创建模块化内容，从而增强一致性和适应性。

首先，我们将指导您在AEM中启用内容片段，包括无缝集成所需的配置和设置。

接下来，我们将介绍如何创建片段模型，其中定义了结构和属性。 了解如何设计符合您的内容要求的模型并有效地管理它们。

然后，我们将演示如何从模型创建内容片段，提供有关创作和发布的分步指南。

此外，我们将探索如何定义AEM GraphQL端点。 GraphQL高效地从AEM检索数据，我们将设置和配置端点以公开所需数据。 持久查询将优化性能和缓存。
...
1. 在文件夹内，选择 __创建__ 并为模型命名 __Teaser__. 将以下数据类型添加到 __Teaser__ 型号。
   ...
   | 枚举 | 样式 | 是 | 呈现为下拉列表。 选项包括Hero -> hero和Featured -> featured |
...
1. 在文件夹的 __云配置__ 选项卡，选择配置 [创建时间较早](#enable-content-fragments-and-graphql).

   ![资源文件夹AEM Headless云配置](./assets/1/cloud-config.png)

   单击进入新文件夹并创建Teaser。 单击 __创建__ 和 __内容片段__ 并选择 __Teaser__ 型号。 为模型命名 __Hero__ 并单击 __创建__.
...
1. 单击 __创建__ 并为新端点指定名称并选择新创建的配置。
...
恭喜！您已成功将AEMas a Cloud Service配置为允许创建内容片段和GraphQL端点。 您还创建了内容片段模型和内容片段，并定义了GraphQL端点和持久查询。 现在，您已准备好进入下一教程章节，其中您将了解如何创建使用您在本章中创建的内容片段和GraphQL端点的AEM Headless React应用程序。
