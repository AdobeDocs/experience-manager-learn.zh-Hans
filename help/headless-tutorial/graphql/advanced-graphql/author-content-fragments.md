...
description: 在本章Adobe Experience Manager (AEM) Headless的高级概念中，了解如何在内容片段中使用选项卡、日期和时间、JSON对象和片段引用。 设置文件夹策略以限制可以包含的内容片段模型。
...
在 [上一章](/help/headless-tutorial/graphql/advanced-graphql/create-content-fragment-models.md)，您已创建五个内容片段模型：人员、团队、位置、地址和联系信息。 本章将指导您完成基于这些模型创建内容片段的步骤。 本教程还探索了如何创建文件夹策略以限制可在文件夹中使用的内容片段模型。

## 前提条件 {#prerequisites}

本文档是多部分教程的一部分。 请确保 [上一章](create-content-fragment-models.md) 已完成，然后才能继续阅读本章内容。
...
安装一个AEM包，该包中包含多个用于加速教程的文件夹和示例图像。
...
从AEM主页中，选择 **资产** > **文件** > **WKND已共享** > **英语**. 在这里，您可以看到各种内容片段类别，包括冒险和投稿人。
...
1. 在打开的“选择路径”对话框中，按照以下路径操作 **会议** > **WKND已共享**. 在上一章中创建的人员内容片段模型包含对联系信息内容片段模型的引用。 必须在“讲师”文件夹中同时允许使用人员和联系信息模型，才能创建讲师内容片段。 选择 **人员** 和 **联系信息**，然后按键 **选择** 以关闭对话框。

   ![选择路径](assets/author-content-fragments/select-path.png)

1. 选择 **保存并关闭** 并选择 **确定** 在显示的成功对话框中。

1. 您现在已为讲师文件夹配置了文件夹策略。 导航到 **讲师** 文件夹并选择 **创建** > **内容片段**. 您现在可以选择的模型只有 **人员** 和 **联系信息**.

   ![文件夹策略](assets/author-content-fragments/folder-policies.png)

## 为讲师创作内容片段

导航到 **讲师** 文件夹。 从这里，让我们创建一个嵌套文件夹，用于存储讲师的联系信息。

请按照以下章节中概述的步骤 [创建文件夹](#create-folders) 创建名为“联系人信息”的文件夹。 嵌套文件夹继承父文件夹的文件夹策略。 请随意配置更具体的策略，以便新创建的文件夹仅允许使用联系信息模型。

### 创建讲师内容片段

让我们创建四个可以添加到冒险讲师团队的人员。

1. 在“讲师”文件夹中，创建基于人员内容片段模型的内容片段，并将其标题命名为“Jacob Wester”。
...
1. 在字段中输入以下内容：
   ...
   * **全名**：雅各布·韦斯特
   * **传记**：雅各布·韦斯特当了10年的远足教练，喜欢这几乎每一分钟！ 雅各布是一名探险家，有攀岩和背包方面的天赋。 雅各布是攀登比赛的赢家，包括海湾之战攀登比赛。 雅各布目前住在加利福尼亚。
   * **讲师经验级别**：专家
   * **技能**：攀岩、冲浪、背包
   * **管理员详细信息**：雅各布·韦斯特三年来一直在协调背包冒险。

1. 在 **配置文件图片** 字段中，添加对图像的内容引用。 浏览到 **WKND已共享** > **英语** > **参与者** > **jacob_wester.jpg** 创建图像的路径。

### 从内容片段编辑器创建片段引用 {#fragment-reference-from-editor}

AEM允许您直接从内容片段编辑器创建片段引用。 让我们引用Jacob的联系信息。

1. 选择 **新内容片段** 在 **联系信息** 字段。

   ![新内容片段](assets/author-content-fragments/new-content-fragment.png)

1. 此时将打开“新内容片段”模式窗口。 在选择目标选项卡下，遵循路径 **冒险** > **讲师** 并选中 **联系信息** 文件夹。 选择 **下一个** 以进入“Properties（属性）”选项卡。

   ![新内容片段模式](assets/author-content-fragments/new-content-fragment-modal.png)

1. 在“属性”选项卡下，在 **标题** 字段。 选择 **创建**，然后按键 **打开** 在显示的成功对话框中。
...
1. 要导航回讲师内容片段，请选择 **雅各布·韦斯特** 在编辑器的左上角。
   ...
   此 **联系信息** 字段现在包含引用联系人信息片段的路径。 这是一个嵌套片段引用。 完成的讲师内容片段如下所示：
...
遵循中概述的相同流程 [上一节](#fragment-reference-from-editor) 创建三个其他讲师内容片段和三个联系信息内容片段。 在讲师片段中添加以下内容：
...
| 配置文件图片 | /content/dam/wknd-shared/en/contributors/stacey-roswells.jpg |
| 传记 | 史黛西·罗斯威尔是一位成功的攀岩家和高山探险家。 斯泰西出生于马里兰州巴尔的摩，是六个孩子中的老幺。 史黛丝的父亲是美国海军中校，母亲是一位现代舞蹈教练。 斯泰西的家人经常因父亲的工作而搬家，并在父亲驻泰国时拍摄了第一张照片。 这也是Stacey学会攀岩的地方。 |
| 讲师经验级别 | 高级 |
| 技能 | 攀岩 |滑雪 |背包 |
...
| 配置文件图片 | /content/dam/wknd-shared/en/contributors/kumar-selvaraj.jpg |
| 传记 | Kumar Selvaraj是一名经验丰富的AMGA认证专业讲师，其主要目标是帮助学生提高攀爬和登山技能。 |
| 讲师经验级别 | 高级 |
| 技能 | 攀岩 |背包 |
...
| 配置文件图片 | /content/dam/wknd-shared/en/contributors/ayo-ogunseinde-237739.jpg |
| 传记 | Ayo Ogunseinde是一名职业登山者和背包教练，居住在中加州的弗雷斯诺。 Ayo的目标是引导徒步旅行者进行他们最壮观的国家公园冒险。 |
| 讲师经验级别 | 高级 |
| 技能 | 攀岩 |自行车 |背包 |
...
暂时忽略Yosemite Valley Lodge文件夹。 在本节的后面部分，当我们创建一个位置，作为我们讲师团队的Home Base时，我们会回顾该位置。

导航到 **约塞米蒂国家公园** 文件夹。 目前，它只包含约塞米蒂国家公园的图片。 让我们使用位置内容片段模型创建一个内容片段，并将其命名为“约塞米蒂国家公园”。
...
此 **位置详细信息** 选项卡包含 **名称**， **描述**， **联系信息**， **位置图像**、和 **按季节显示天气** 字段，而 **位置地址** 选项卡包含对地址内容片段的引用。 通过选项卡，可以清楚了解必须填写哪些类型的内容，因此创作内容更易于管理。
...
1. 在 **位置详细信息** 选项卡，请在字段中输入以下信息：

   * **名称**：约塞米蒂国家公园
   * **描述**：约塞米蒂国家公园位于加利福尼亚的内华达山脉。 它以华丽的瀑布、巨大的红杉树以及埃尔卡皮坦半圆顶悬崖的标志性景色而闻名。 徒步旅行和露营是体验约塞米蒂的最佳方式。 许多小径为探险和探索提供了无限的机会。

1. 从 **联系信息** 字段中，根据联系人信息模型创建一个内容片段，并将其命名为“Yosemite National Park Contact Info”。 遵循上一节中概述的相同流程 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在字段中输入以下数据：

   * **电话**：209-999-0000
   * **电子邮件**： yosemite@wknd.com

1. 从 **位置图像** 字段，浏览至 **冒险** > **位置** > **约塞米蒂国家公园** > **约塞米蒂 — 国家公园.jpeg** 创建图像的路径。

   请记住，在上一章中，您配置了图像验证，因此“位置”图像的尺寸必须小于2560 x 1800，并且其文件大小必须小于3 MB。

1. 添加所有信息后， **位置详细信息** 选项卡现在看起来像这样：

   ![位置详细信息选项卡已完成](assets/author-content-fragments/location-details-tab-completed.png)

1. 导航到 **位置地址** 选项卡。 从 **地址** 字段，使用您在上一章中创建的地址内容片段模型创建名为“Yosemite National Park Address”的内容片段。 遵循以下章节中概述的相同流程 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在字段中输入以下数据：
...
1. 导航到 **约塞米蒂谷旅馆** 文件夹。 使用位置内容片段模型创建一个内容片段，并将其命名为“Yosemite Valley Lodge”。

1. 在 **位置详细信息** 选项卡，请在字段中输入以下信息：

   * **名称**：约塞米蒂谷旅馆
   * **描述**：约塞米蒂山谷旅馆是团体会议和各种活动（如购物、用餐、钓鱼、远足等）的枢纽。

1. 从 **联系信息** 字段中，根据联系人信息模型创建一个内容片段，并将其命名为“Yosemite Valley Lodge联系人信息”。 遵循以下章节中概述的相同流程 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在新内容片段的字段中输入以下数据：
...
1. 导航回到 **约塞米蒂谷旅馆** 然后转到 **位置地址** 选项卡。 从 **地址** 字段，使用您在上一章中创建的地址内容片段模型创建名为“Yosemite Valley Lodge Address”的内容片段。 遵循以下章节中概述的相同流程 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在字段中输入以下数据：
...
1. 首先，将以下文本添加到 **描述** “在约塞米蒂国家公园工作的专业冒险家和登山教官团队。”

1. 要添加内容引用，请选择 **插入资源** 图标。

   ![插入资源图标](assets/author-content-fragments/insert-asset-icon.png)

1. 在出现的模式窗口中，选择 **team-yosemite-logo.png** 并按 **选择**.
...
1. 浏览到 **WKND已共享** > **英语** > **冒险** > **位置** > **约塞米蒂谷旅馆** > **约塞米蒂谷旅馆**. 按 **选择** 以插入内容片段。
...
让我们看一下“日期和时间”数据类型。 选择 **日历** 图标（位于的右侧） **团队创建日期** 用于打开日历视图的字段。

![日期日历视图](assets/author-content-fragments/date-calendar-view.png)

可以使用月份任一侧的向前和向后箭头设置过去或未来的日期。 比如，约塞米蒂国家队是在2016年5月24日成立的，那么我们将确定成立日期。
...
1. 选择 **添加** 再按三次。 使用新字段将剩余的三个讲师添加到团队。 此 **团队成员** 字段现在如下所示：
...
1. 在中添加片段引用 **讲师团队** 字段。 团队路径应引用您创建的Yosemite团队内容片段： `/content/dam/wknd-shared/en/adventures/teams/yosemite-team/yosemite-team`. 这是一个嵌套片段引用。 团队内容片段包含对人员模型的引用，其中引用了联系信息和地址模型。 因此，您的嵌套内容片段向下分为三级。

1. 在中添加片段引用 **管理员** 字段。 假设雅各布·韦斯特是Yosemite Backpacking Adventure的管理员。 路径应该指向Jacob Wester内容片段，并显示如下： `/content/dam/wknd-shared/en/adventures/instructors/jacob-wester`.

1. 您现在已将三个片段引用添加到冒险内容片段。 这些字段如下所示：
...
恭喜！您现在已根据上一章中创建的高级内容片段模型创建了内容片段。 您还创建了一个文件夹策略，以限制可以在文件夹中选择什么内容片段模型。

## 后续步骤

在 [下一章节](/help/headless-tutorial/graphql/advanced-graphql/explore-graphql-api.md)，您将了解有关使用GraphiQL集成开发环境(IDE)发送高级GraphQL查询的信息。 这些查询允许我们查看在本章中创建的数据，并最终将这些查询添加到WKND应用程序。
