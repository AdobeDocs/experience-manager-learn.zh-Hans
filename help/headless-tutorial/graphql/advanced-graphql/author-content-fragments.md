...
本文档是多部分教程的一部分。 请确保 [上一章节](create-content-fragment-models.md) 已完成，然后才能继续处理本章。
...
从AEM主页中，选择 **资产** > **文件** > **WKND共享** > **英语**. 在这里，您可以看到各种内容片段类别，包括冒险和参与者。
...
1. 选择 **策略** 选项卡，然后取消选择 **继承自/content/dam/wknd-shared**. 在 **允许的内容片段模型（按路径）** 字段中，选择文件夹图标。

   ![文件夹图标](assets/author-content-fragments/folder-icon.png)

1. 在打开的“选择路径”对话框中，按照路径 **conf** > **WKND共享**. 在上一章中创建的人员内容片段模型包含对联系信息内容片段模型的引用。 必须在Aditors文件夹中允许使用“人员”和“联系信息”模型，才能创建教师内容片段。 选择 **人员** 和 **联系信息**，然后按 **选择** 来关闭对话框。
...
按照 [创建文件夹](#create-folders) 创建标题为“联系信息”的文件夹。 嵌套文件夹会继承父文件夹的文件夹策略。 请随时配置更具体的策略，以便新创建的文件夹仅允许使用联系信息模型。

### 创建讲师内容片段

让我们创建四个可添加到冒险指导团队的人员。

1. 从“指导者”文件夹中，根据“人员内容片段模型”创建内容片段，并为其指定标题“Jacob Wester”。
...
1. 在字段中输入以下内容：
   ...
   * **全名**:雅各布·韦斯特
   * **传记**:雅各布·韦斯特是徒步教练10年了，每时每刻都很喜欢！ 雅各布是一位探险家，擅长攀岩和背包。 雅各布是攀岩比赛的胜者，包括海湾巨石竞赛。 雅各目前住在加州。
   * **讲师体验级别**:专家
   * **技能**:攀岩、冲浪、背包
   * **管理员详细信息**:雅各布·韦斯特已经协调了三年的背包冒险。

1. 在 **配置文件图片** 字段中，添加对图像的内容引用。 浏览到 **WKND共享** > **英语** > **参与者** > **jacob_wester.jpg** 创建图像路径。

### 从内容片段编辑器中创建片段引用 {#fragment-reference-from-editor}
...
| 内容片段标题 | 史黛西·罗斯威尔斯 |
| 完整名称 | 史黛西·罗斯威尔斯 |
| 联系信息 | /content/dam/wknd-shared/en/adventures/auditers/contact-info/stacey-roswells-contact-info |
| 配置文件图片 | /content/dam/wknd-shared/en/contributors/stacey-roswells.jpg |
| 传记 | 史黛西·罗斯威尔斯是一位杰出的攀岩者和高山探险家。 斯泰西出生于马里兰州巴尔的摩，是六个孩子中的老幺。 史黛西的父亲是美国海军中校，母亲是现代舞教师。 斯泰西的家人经常随父亲的任务搬走，父亲在泰国驻扎时，第一张照片就拍了。 这也是史黛西学会攀岩的地方。 |
| 讲师体验级别 | 高级 |
| 技能 | 攀岩 |滑雪 |回装 |
...
| 内容片段标题 | 库马尔·塞尔瓦拉伊 |
| 完整名称 | 库马尔·塞尔瓦拉伊 |
| 联系信息 | /content/dam/wknd-shared/en/adventures/auditers/contact-info/kumar-selvaraj-contact-info |
...
| 内容片段标题 | 奥贡辛德代 |
| 完整名称 | 奥贡辛德代 |
| 联系信息 | /content/dam/wknd-shared/en/adventures/auditors/contact-info/ayo-ogunseinde-contact-info |
| 配置文件图片 | /content/dam/wknd-shared/en/contributors/ayo-ogunseinde-237739.jpg |
| 传记 | Ayo Ogunseinde是一名职业登山者和背包教练，住在加州中部弗雷斯诺。 Ayo的目标是引导徒步旅行者进行他们最史诗般的国家公园冒险。 |
| 讲师体验级别 | 高级 |
| 技能 | 攀岩 |循环 |回装 |
...
暂时忽略Yosemite Valley Lodge文件夹。 当我们创建一个位置作为指导团队的“主基地”时，我们将在本节的后面部分返回该位置。

导航到 **约塞米蒂国家公园** 文件夹。 目前，它只包含约塞米蒂国家公园的一幅图片。 让我们使用位置内容片段模型创建内容片段，并将其命名为“Yosemite National Park”。
...
1. 从 **联系信息** 字段中，根据联系信息模型创建内容片段并将其命名为“Yosemite National Park联系信息”。 遵循上一节中所述的相同流程 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在字段中输入以下数据：
...
1. 从 **位置图像** 字段，浏览 **冒险** > **位置** > **约塞米蒂国家公园** > **yosemite-national-park.jpeg** 创建图像路径。
   ...
   请记住，在上一章中，您配置了图像验证，因此位置图像的尺寸必须小于2560 x 1800，并且其文件大小必须小于3 MB。

1. 添加所有信息后， **位置详细信息** 选项卡现在如下所示：

   ![“位置详细信息”选项卡已完成](assets/author-content-fragments/location-details-tab-completed.png)

1. 导航到 **位置地址** 选项卡。 从 **地址** 字段中，使用您在上一章中创建的地址内容片段模型，创建标题为“Yosemite国家公园地址”的内容片段。 按照 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在字段中输入以下数据：
...
1. 从 **联系信息** 字段中，根据“联系信息”模型创建内容片段，并将其命名为“Yosemite Valley Lodge联系信息”。 按照 [从编辑器创建片段引用](#fragment-reference-from-editor) ，然后在新内容片段的字段中输入以下数据：
...
1. 导航回 **约塞米蒂谷旅馆** 然后转到 **位置地址** 选项卡。 从 **地址** 字段中，使用您在上一章中创建的地址内容片段模型，创建标题为“Yosemite Valley Lodge Address”的内容片段。 按照 [从编辑器创建片段引用](#fragment-reference-from-editor) 并在字段中输入以下数据：
...
让我们使用团队内容片段模型创建内容片段，并将其命名为“Yosemite Team”。
...
1. 浏览到 **WKND共享** > **英语** > **冒险** > **位置** > **约塞米蒂谷旅馆** > **约塞米蒂谷旅馆**. 按 **选择** 来插入内容片段。
...
1. 在显示的新字段中，选择文件夹图标以打开“选择路径”模式。 浏览文件夹以 **WKND共享** > **英语** > **冒险** > **讲师**，然后选中旁边的复选框 **雅各布 — 韦斯特**. 按 **选择** 来保存路径。
...
1. 在 **位置** 字段。 位置路径应引用您创建的Yosemite国家公园内容片段： `/content/dam/wknd-shared/en/adventures/locations/yosemite-national-park/yosemite-national-park`.

1. 在 **讲师团队** 字段。 团队路径应引用您创建的Yosemite团队内容片段： `/content/dam/wknd-shared/en/adventures/teams/yosemite-team/yosemite-team`. 这是嵌套的片段引用。 团队内容片段包含引用联系信息和地址模型的人员模型的引用。 因此，您在三个级别下设置了嵌套内容片段。

1. 在 **管理员** 字段。 比如说，雅各布·韦斯特是约塞米蒂背包冒险的管理员。 该路径应会导向Jacob Wester内容片段，并如下所示： `/content/dam/wknd-shared/en/adventures/instructors/jacob-wester`.
...
在 [下一章](/help/headless-tutorial/graphql/advanced-graphql/explore-graphql-api.md)，您将了解如何使用GraphiQL集成开发环境(IDE)发送高级GraphQL查询。 这些查询允许我们查看本章中创建的数据，并最终将这些查询添加到WKND应用程序。
