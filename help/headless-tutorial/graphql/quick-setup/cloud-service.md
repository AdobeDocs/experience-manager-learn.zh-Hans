...
description: AEM Headless快速设置允许您使用AEM Headless和React应用程序，前者使用WKND Site示例项目中的内容，后者通过AEM Headless GraphQL API使用内容。
...
+ 访问AEMas a Cloud Service和Cloud Manager
   + __AEM管理员__ 对AEMas a Cloud Service的访问权限
...
1. 运行 __开发部署管道__
   ...
   1. 选择 __查看详细信息__
1. 从管道执行的详细信息中，监控进度，直到成功完成。 管道执行应需要30-40分钟。
...
1. 在本地运行的React应用程序启动时 [http://localhost:3000](http://localhost:3000) 和显示冒险列表，这些冒险是使用AEM Headless的GraphQL API从AEMas a Cloud Service获得的。
...
1. 打开 __骑车去犹他州南部__ 文件夹
1. 选择 __骑车去犹他州南部__ 内容片段，并选择 __编辑__ 从顶部操作栏中
1. 更新内容片段的某些字段，例如：
