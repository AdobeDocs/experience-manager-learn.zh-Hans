...
description: 了解AEM Author和Publish服务以及针对Headless应用程序推荐的部署模式。 在本教程中，了解如何使用环境变量根据目标环境动态更改GraphQL端点。 了解如何正确配置AEM以进行跨源资源共享(CORS)。
...
# 使用AEM Publish服务进行生产部署

在本教程中，您将设置一个本地环境来模拟将内容从Author实例分发到Publish实例。 您还将生成一个React应用程序的生产版本，该应用程序配置为使用GraphQL API从AEM Publish环境中使用内容。 您将逐渐学会如何有效地使用环境变量以及如何更新AEM CORS配置。
...
## 创作Publish部署模式 {#deployment-pattern}
...
本教程通过在当前设置中添加AEM Publish实例来模拟上述部署。 在前面的章节中，React应用程序通过直接连接到Author实例来充当预览。 React应用程序的生产内部版本将部署到连接到新Publish实例的静态Node.js服务器。
...
## 安装AEM SDK - Publish模式 {#aem-sdk-publish}
...
1. 在本地文件系统上，创建一个专用文件夹来安装Publish实例，即名为`~/aem-sdk/publish`。
...
1. 将jar文件重命名为`aem-publish-p4503.jar`。
   ...
   `publish`字符串指定快速入门Jar以Publish模式启动。 `p4503`指定快速入门服务器在端口4503上运行。
...
1. 当AEM实例安装完成后，将在[http://localhost:4503/content.html](http://localhost:4503/content.html)打开一个新的浏览器窗口

   应返回“404未找到”页面。 这是一个全新的AEM实例，尚未安装任何内容。

## 安装示例内容和GraphQL端点 {#wknd-site-content-endpoints}

就像在创作实例上一样，Publish实例需要启用GraphQL端点，并需要示例内容。 接下来，在Publish实例上安装WKND参考站点。
...
1. 通过直接导航到[http://localhost:4503/libs/granite/core/content/login.html](http://localhost:4503/libs/granite/core/content/login.html)登录Publish实例，其中用户名为`admin`，密码为`admin`。
...
接下来，更新React应用程序使用的环境变量以指向Publish实例。 React应用程序应&#x200B;**仅**&#x200B;连接到生产模式的Publish实例。
...
1. 使用以下内容填充`.env.production.local`：
   ...
   >[!NOTE]
   >
   > 请注意，不包含身份验证信息，因为Publish环境默认提供对内容的匿名访问。
...
1. 打开新浏览器并导航到[http://localhost:5000/](http://localhost:5000/)。 您应该会看到正在提供的React应用程序。
   ...
   请注意，GraphQL查询正在主页上运行。 使用您的开发人员工具Inspect **XHR**&#x200B;请求。 请注意，GraphQLPOST位于`http://localhost:4503/content/graphql/global/endpoint.json`处的Publish实例。
...
图像似乎已损坏，因为`<img src`属性设置为相对路径，最终指向`http://localhost:5000/`处的Node静态服务器。 这些图像应该指向AEM Publish实例。 对此有多种可能的解决方案。 使用webpack开发服务器时，文件`react-app/src/setupProxy.js`在webpack服务器和AEM创作实例之间为对`/content`的任何请求设置代理。 代理配置可以在生产环境中使用，但必须在Web服务器级别进行配置。 例如，[Apache的代理模块](https://httpd.apache.org/docs/2.4/mod/mod_proxy.html)。
...
请记住，在请求“冒险详细信息”页面时，`adventureContributor`引发了GraphQL错误。 Publish实例上尚不存在&#x200B;**参与者**&#x200B;内容片段模型。 对&#x200B;**Adventure**&#x200B;内容片段模型所做的更新在Publish实例上不可用。 这些更改是直接针对创作实例做出的，需要分发到Publish实例。

在将新更新转出到依赖于内容片段或内容片段模型的更新的应用程序时，需要考虑这一点。

接下来，用于模拟本地Author实例和Publish实例之间的内容发布。
...
1. 下载包[EnableReplicationAgent.zip](./assets/publish-deployment/EnableReplicationAgent.zip)并使用包管理器安装它。
   ...
   此包将安装一种配置，该配置允许作者实例将内容发布到Publish实例。 可以在此处](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/aem-runtime.html?lang=en#content-distribution)找到[此配置的手动步骤。

   >[!NOTE]
   >
   > 在AEM as a Cloud Service环境中，创作层自动设置为将内容分发到Publish层。
...
1. 选择所有三个模型并单击&#x200B;**Publish**：
   ...
   ![Publish内容片段模型](assets/publish-deployment/publish-contentfragment-models.png)
...
1. 单击顶部菜单栏中的&#x200B;**Publish**&#x200B;按钮。
   ...
   ![在内容片段编辑器中单击Publish按钮](assets/publish-deployment/publish-bali-content-fragment.png)

1. Publish向导会显示应发布的任何依赖关系资源。 在这种情况下，将列出引用的片段&#x200B;**stacey-roswells**，并且还将引用多个图像。 引用的资产会与片段一起发布。

   ![已引用要发布的Assets](assets/publish-deployment/referenced-assets.png)

   再次单击&#x200B;**Publish**&#x200B;按钮可发布内容片段和从属资源。
...
1. 返回到[http://localhost:5000/adventure:/content/dam/wknd/en/adventures/bali-surf-camp/bali-surf-camp](http://localhost:5000/adventure:/content/dam/wknd/en/adventures/bali-surf-camp/bali-surf-camp)并观察发布的更改。
   ...
   ![巴厘岛冲浪营Publish更新](assets/publish-deployment/bali-surf-camp-update.png)

## 更新COR配置

默认情况下，AEM是安全的，不允许非AEM Web属性进行客户端调用。 AEM跨源资源共享(CORS)配置允许特定域调用AEM。
...
1. 导航到以[http://192.168.86.XXX:5000](http://192.168.86.XXX:5000)开头的地址。 每台本地计算机的地址稍有不同。 请注意，获取数据时存在CORS错误。 这是因为当前CORS配置仅允许来自`localhost`的请求。
   ...
   接下来，更新AEM Publish CORS配置，以允许来自网络IP地址的请求。
...
1. 搜索&#x200B;**Apache Sling引用过滤器**&#x200B;并查看配置。 还需要配置&#x200B;**允许为空**&#x200B;以启用来自外部域的GraphQL请求。
   ...
   >[!NOTE]
   >
   > OSGi配置在提交到源代码控制的AEM项目中管理。 可以使用Cloud Manager将AEM项目作为Cloud Service环境部署到AEM。 [AEM项目原型](https://github.com/adobe/aem-project-archetype)可以帮助为特定实现生成项目。
