---
title: 内容片段创作 — AEM Headless入门 — GraphQL
...
这是一个多部分教程，并假定中概述的步骤 [定义内容片段模型](./content-fragment-models.md) 已完成。
...
内容片段存储在AEM Assets的文件夹中。 要从上一章中创建的模型创建内容片段，必须创建文件夹以存储它们。 需要对该文件夹进行配置才能从特定模型创建片段。
...
1. 点按 **Cloud Services** 选项卡。 在云配置选项卡下，使用路径查找器选择 **我的项目** 配置。 该值应为 `/conf/my-project`.
...
1. 点按 **策略** 选项卡，在 **允许的内容片段模型** 字段使用路径查找器选择 **人员** 和 **团队** 之前创建的模型。
   ...
   任何子文件夹都会自动继承这些策略，并且可以覆盖这些策略。 您还可以按标记允许模型或从其他项目配置启用模型。 此机制提供了一种管理内容层次结构的强大方法。
...
1. 使用以下值创建另一个文件夹：
   ...
   * 名称： **en**
   ...
   最佳实践是为多语言支持设置项目。 参见 [有关更多信息，请参阅以下文档页面](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/admin/translate-assets.html).
...
1. 从AEM开始屏幕，点按 **内容片段** 以打开内容片段UI。
   ...
   ![内容片段用户界面](assets/author-content-fragments/cf-fragment-ui.png)

1. 在左侧边栏中，展开 **我的项目** 并点按 **英语**.
1. 点按 **创建** 以调出 **新内容片段** 对话框并输入以下值：
...
1. 接下来，重复这些步骤以创建 **团队** 片段表示 **Alpha团队**：
...
1. 点按 **保存并关闭** 以保存对Team Alpha片段的更改。
...
在下一章中， [探索GraphQL API](explore-graphql-api.md)，您将使用内置的GrapiQL工具探索AEM GraphQL API。 了解AEM如何根据内容片段模型自动生成GraphQL架构。 您将尝试使用GraphQL语法构建基本查询。
