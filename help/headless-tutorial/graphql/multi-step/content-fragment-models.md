...
description: Adobe Experience Manager (AEM)和GraphQL快速入门。 了解如何在AEM中使用内容片段模型为内容建模并构建架构。 查看现有模型并创建模型。 了解可用于定义架构的不同数据类型。
...
在本章中，了解如何使用为内容建模并构建架构 **内容片段模型**. 了解可用于将架构定义为模型一部分的不同数据类型。

我们创建了两个简单的模型， **团队** 和 **人员**. 此 **团队** 数据模型具有名称、简短名称和描述，并引用 **人员** 数据模型，其中包含全名、个人资料详细信息、个人资料图片和职业列表。

此外，还欢迎您按照基本步骤创建自己的模型，并调整相应的步骤，如GraphQL查询和React应用程序代码，或只是按照这些章节中概述的步骤进行操作。

## 前提条件 {#prerequisites}

这是一个多部分教程，并假定包含 [AEM创作环境可用](./overview.md#prerequisites).
...
1. 在生成的对话框中，输入：
   ...
   * 标题*： **我的项目**
   * 名称*： **my-project** (偏好使用全部小写字母和连字符来分隔单词。 此字符串会影响客户端应用程序执行请求的唯一GraphQL端点。)
   * Check **内容片段模型**
   * Check **GraphQL持久查询**
...
1. 导航到 **我的项目** 文件夹。
1. 点按 **创建** 在右上角，打开 **创建模型** 向导。
1. In **模型标题** 字段，输入 **人员** 并点按 **创建**. 在生成的对话框中，点按 **打开**，以构建模型。

1. 拖放 **单行文本** 元素显示在主面板上。 在 **属性** 选项卡：
   ...
   * Check **必需**
   ...
   此 **属性名称** 定义保留到AEM的属性的名称。 此 **属性名称** 还会定义 **键** 作为数据架构一部分的此属性的名称。 此 **键** 通过GraphQL API公开内容片段数据时使用。

1. 点按 **数据类型** 制表符并拖放 **多行文本** 字段的下方 **全名** 字段。 输入以下属性：
...
1. 单击 **数据类型** 制表符并拖放 **内容引用** 字段。 输入以下属性：
   ...
   * **根路径**: `/content/dam`
   ...
   配置时 **根路径**，您可以单击 **文件夹** 图标以调出用于选择路径的模式。 这会限制作者可以使用哪些文件夹填充路径。 `/content/dam` 是存储所有AEM Assets（图像、视频和其他内容片段）的根。
...
1. 添加多个 **选项** 使用 **添加选项** 按钮。 对使用相同的值 **选项标签** 和 **选项值**：
   ...
   **艺术家**， **影响因素**， **摄影师**， **Traveler**， **作者**， **YouTuber**
...
为创建模型 **团队**，这是一组人员的数据模型。 “团队”模型引用“人员”模型来表示团队成员。

1. 在 **我的项目** 文件夹，点按 **创建** 在右上角显示 **创建模型** 向导。
1. In **模型标题** 字段，输入 **团队** 并点按 **创建**.
...
1. 点按 **数据类型** 制表符并拖放 **单行文本** 元素显示在主面板上。 在 **属性** 选项卡：
   ...
   * Check **独特**
   * 在下， **验证类型** >选择 **自定义**
   * 在下， **自定义验证正则表达式** > enter `^[a-z0-9\-_]{5,40}$`  — 这确保只能输入5到40个字符的小写字母数字值和破折号。

   此 `shortName` 属性为我们提供了一种根据缩短路径查询单个团队的方法。 此 **独特** 设置可确保该值对于此模型的每个内容片段始终是唯一的。

1. 点按 **数据类型** 制表符并拖放 **多行文本** 字段的下方 **简短名称** 字段。 输入以下属性：
...
在下一章中， [创作内容片段模型](author-content-fragments.md)中，您可以根据内容片段模型创建和编辑新的内容片段。 您还将了解如何创建内容片段的变体。
