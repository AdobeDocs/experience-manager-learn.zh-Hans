...
本教程使用 **简单的本机Android移动设备应用程序** 以使用和显示AEM Content Services公开的事件内容。

使用 [Android](https://developer.android.com/) 这在很大程度上并不重要，而且消费型移动设备应用程序可以在任何移动平台(例如iOS)的框架中编写。

Android用于教程，是因为其可以在Windows、macOs和Linux上运行Android模拟器，其受欢迎程度以及它可以编写为Java，AEM开发人员非常了解该语言。

*本教程的Android移动设备应用程序是&#x200B;**非**旨在指导如何构建Android移动设备应用程序或传达Android开发最佳实践，但并非说明如何从移动设备应用程序使用AEM内容服务。*
...
如果AEM发布未在上运行 **http://localhost:4503** 主机和端口可以在移动设备应用程序的 [!DNL Settings] 指向属性AEM发布主机/端口。
...
1. 打开 **Android Studio**
   * 首次启动Android Studio时，系统会提示安装 [!DNL Android SDK] 将出席。 接受默认值并完成安装。
...
1. 首次启动Android Studio时，右键单击 **wknd-mobile.x.x.x** 在“项目”列表中，选择 **打开模块设置**.
   ...
   * 如果不这样做，则在尝试启动模拟器时将看到“请选择Android SDK”错误。
...
1. 点按 **运行** 按钮来打开所选内容 **运行/编辑配置**
1. 在弹出窗口中，选择新创建的 **[!DNL Pixel 2 API 29]** 虚拟设备和点击 **确定**
1. 如果 [!DNL WKND Mobile] 应用程序不会立即加载、查找并点按 **[!DNL WKND]** 图标图标。
...
加载后，移动设备应用程序会发出 `HTTP GET` 到 `/content/wknd-mobile/en/api/events.model.json` ，它是指配置为提供内容以驱动移动设备应用程序的AEM内容服务端点。

因为事件API的可编辑模板(`/content/wknd-mobile/en/api/events.model.json`)锁定，则可以对移动设备应用程序进行编码，以在JSON响应中的特定位置查找特定信息。

### 高级代码流

1. 打开 [!DNL WKND Mobile] 应用程序调用 `HTTP GET` 请求发布至AEM Publish `/content/wknd-mobile/en/api/events.model.json` 收集内容以填充移动设备应用程序的UI。
2. 从AEM收到内容后，移动设备应用程序的三个视图元素中的每个 **徽标、标签行和活动列表**，使用AEM中的内容进行初始化。
   * 要将AEM内容绑定到移动设备应用程序的视图元素，表示每个AEM组件的JSON是映射到Java POJO的对象，而该Java POJO又绑定到Android视图元素。
      * 图像组件JSON →徽标POJO →徽标图像视图
      * 文本组件JSON → TagLine POJO →文本图像视图
      * 内容片段列表JSON →事件POJO →事件RecycleView
...
以下是移动设备应用程序的 `MainActivity` 负责调用AEM Content Services以收集推动移动设备应用程序体验的内容。
...
`initApp(...)` 随后调用，这会向AEM Publish上的AEM Content Services端点发出HTTPGET请求以收集内容。 收到有效的JSON响应后，JSON响应将传递到每个 `ViewBinder` 负责解析JSON并将其绑定到移动设备 `View` 元素。

#### 解析JSON响应

接下来，我们将查看 `LogoViewBinder`，很简单，但会强调几个重要注意事项。
...
第一行 `bind(...)` 通过键向下导航JSON响应 **：items → root → ：items** ，表示组件添加到的AEM布局容器。

在此处，将对名为的密钥进行检查 **图像**，它表示图像组件(同样，此节点名称→JSON键稳定也很重要)。 如果此对象存在，则它读取并映射到 [自定义图像POJO](#image-pojo) 通过杰克逊 `ObjectMapper` 库。 图像POJO如下所述。

最后，徽标的 `src` 使用加载到Android ImageView中 [!DNL Glide] 帮助程序库。
...
虽然可选，但使用 [Jackson对象映射器](https://fasterxml.github.io/jackson-databind/javadoc/2.9/com/fasterxml/jackson/databind/ObjectMapper.html) 其他库（如Gson）提供的类似功能有助于将复杂的JSON结构映射到Java POJO，而无需直接处理本机JSON对象本身。 在这个简单的例子中，我们将 `src` 键自 `image` JSON对象，到 `src` 属性直接通过 `@JSONProperty` 注释。
...
事件POJO需要从JSON对象中选择更多数据点，它比简单的图像更有用，我们只想要简单的图像 `src`.

## 探索移动应用程序体验

现在，您已了解AEM Content Services如何推动本机移动体验，请使用所学知识执行以下步骤，并查看您的更改在移动应用程序中反映的情况。

在每个步骤后，提取以刷新移动设备应用程序，并验证移动设备体验的更新。
