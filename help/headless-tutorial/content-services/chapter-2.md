...
description: AEM Headless教程的第2章介绍了如何启用和定义内容片段模型，这些模型用于定义规范化的数据结构和用于创建事件的创作界面。
...
AEM内容片段模型定义了内容架构，可用于对AEM作者创建的原始内容进行模板化。 这种方法类似于基架或基于表单的创作。 内容片段的关键概念是创作的内容与呈现无关，这意味着它可用于多渠道使用，其中消费应用程序(是AEM、单页应用程序或移动设备应用程序)控制向用户显示内容的方式。
...
如果内容片段模型为 **非** 已为配置启用， **[!UICONTROL 创建] > [!UICONTROL 内容片段]** 按钮不会针对相关的AEM配置显示。
...
>
>要使配置影响内容层次结构，必须通过引用配置 `cq:conf` 属性。 (这是为实现 [!DNL WKND Mobile] 中的配置 **步骤5** （如下所示）。
>
>当 `global` 使用配置，配置应用于所有内容，以及 `cq:conf` 无需设置。
>
>请参阅 [[!UICONTROL 配置浏览器] 文档](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/configurations.html) 了解更多信息。

1. 以具有适当权限的用户身份登录AEM作者以修改相关配置。
   * 在本教程中， **管理员** 可以使用用户。
1. 导航到 **[!UICONTROL 工具] > [!UICONTROL 常规] > [!UICONTROL 配置浏览器]**
1. 点按 **文件夹图标** 旁边 **[!DNL WKND Mobile]** 以选择，然后点按 **[!UICONTROL 编辑] 按钮** 左上角。
1. 选择 **[!UICONTROL 内容片段模型]**，然后点按 **[!UICONTROL 保存并关闭]** 右上角。

   这将在具有以下特征的资产文件夹内容树上启用内容片段模型 [!DNL WKND Mobile] 已应用配置。
...
1. 应用 **[!DNL WKND Mobile]** 配置 **[!DNL WKND Mobile]资产文件夹** 要允许在该资产文件夹层次结构中创建内容片段模型中的内容片段，请执行以下操作：
   ...
   1. In [!UICONTROL 文件夹属性]，点按 **[!UICONTROL Cloud Services]** 选项卡
...
在定义内容片段模型之前，让我们回顾一下将推动的体验，以确保捕获所有必要的数据点。 为此，我们将审查移动设备应用程序设计，并将设计元素映射到要收集的内容。
...
1. 输入 **[!DNL Event]** 作为 **[!UICONTROL 模型标题]** *（说明是可选的）* 并点按 **[!UICONTROL 创建]** 以保存。
...
1. 选择 **[!DNL Event]** 内容片段模型并点按 **[!UICONTROL 编辑]** 在顶部操作栏中。
1. 从 **[!UICONTROL 数据类型] 选项卡** 在右侧，拖动 **[!UICONTROL 单行文本输入]** 放入左侧放置区域中，以定义 **[!DNL Question]** 字段。
1. 确保新的 **[!UICONTROL 单行文本输入]** 左侧选中，并且 **[!UICONTROL 属性] 选项卡** ，则会在右侧选中。 按如下方式填充“属性”字段：
...
>[!NOTE]
>
>此 **[!UICONTROL 属性名称]** 表示 **两者** 存储了此值的JCR属性名称以及JSON文件中的键。 这应该是一个语义名称，在内容片段模型的生命周期内不会发生更改。
...
（可选）安装 [com.adobe.aem.guides.wknd-mobile.content.chapter-2.zip](https://github.com/adobe/aem-guides-wknd-mobile/releases/latest) AEM作者上的内容包，通过 [AEM包管理器](http://localhost:4502/crx/packmgr/index.jsp). 此资源包包含本教程这一部分中概述的配置和内容。
