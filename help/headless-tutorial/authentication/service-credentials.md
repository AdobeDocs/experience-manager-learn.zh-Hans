...
与AEM作为Cloud Service的集成必须能够安全地验证到AEM。 AEM开发人员控制台授予对服务凭据的访问权限，服务凭据用于帮助外部应用程序、系统和服务通过HTTP与AEM作者或发布服务进行有序交互。
...
__这是作为Cloud Service环境的每AEM一次性初始化__

1. 确保您以AdobeIMS组织的管理员身份登录
1. 登录[Adobe云管理器](https://my.cloudmanager.adobe.com)
1. 打开包含AEM的项目作为Cloud Service环境，以集成为
1. 点按&#x200B;__环境__&#x200B;部分环境旁的省略号，然后选择&#x200B;__开发人员控制台__
1. 点按&#x200B;__集成__&#x200B;选项卡
1. 点按&#x200B;__获取服务凭据__&#x200B;按钮
1. 服务凭据将被初始化并显示为JSON
...
下载服务凭据的步骤与初始化的步骤相同。 如果尚未进行初始化，则用户点击&#x200B;__获取服务凭据__&#x200B;按钮时将出现错误。
...
服务凭据提供生成JWT所需的详细信息，JWT将交换为用于与AEM进行身份验证的访问令牌作为Cloud Service。 服务凭据必须存储在外部应用程序、系统或服务访问AEM时可访问的安全位置。 服务凭据的管理方式和位置对于每个客户来说都是独一无二的。

为了简单起见，本教程通过命令行传递服务凭据，但请与IT安全团队合作，了解如何根据组织的安全准则存储和访问这些凭据。

1. 将下载的[服务凭据JSON](#download-service-credentials)复制到项目根目录中名为`service_token.json`的文件
   + 但请记住，千万不要向Git提交任何凭据！

## 使用服务凭据

服务凭据是一个完全形式的JSON对象，它与JWT和访问令牌不同。 而是使用服务凭据（包含私钥）生成JWT，该JWT与AdobeIMS API交换给访问令牌。

![服务凭据——外部应用程序](assets/service-credentials/service-credentials-external-application.png)

1. 将服务凭据从AEM开发人员控制台下载到安全位置
1. 外部应用程序需要以编程方式与AEM作为Cloud Service环境交互
1. 外部应用程序从安全位置读取服务凭据
1. 外部应用程序使用服务凭据中的信息来构建JWT令牌
1. JWT令牌被发送到AdobeIMS以交换访问令牌
1. AdobeIMS返回一个访问令牌，它可以用作访问AEM的Cloud Service
   + 访问令牌可以请求过期。 最好让访问令牌的生命保持短暂，并在需要时更新。
1. 外部应用程序将HTTP请求作为Cloud Service发送到AEM，并将访问令牌作为承载令牌添加到HTTP请求的授权头
1. AEM作为Cloud Service接收HTTP请求、验证请求并执行HTTP请求所请求的工作，并将HTTP响应返回给外部应用程序

### 对外部应用程序的更新

要使用服务凭据以Cloud Service形式访问AEM，必须通过3种方式更新外部应用程序：

1. 在服务凭据中读取
   + 为了简单起见，我们将从下载的JSON文件中读取这些凭据，但在实际使用情况下，必须根据组织的安全准则安全地存储服务凭据
1. 从服务凭据生成JWT
1. 将JWT交换为访问令牌
   + 当存在服务凭据时，当访问AEM作为访问令牌时，我们的外部应用程序使用此访问令牌而不是本地开发Cloud Service

在本教程中，Adobe的`@adobe/jwt-auth` npm模块用于两者，(1)从服务凭据生成JWT,(2)在单个函数调用中将它交换给访问令牌。 如果您的应用程序不基于JavaScript，请查看其他语言的[示例代码，了解如何从服务凭据创建JWT，并将它交换为具有AdobeIMS的访问令牌。](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/JWT/samples/samples.md)

## 阅读服务凭据

查看`getCommandLineParams()`，并查看可以使用与本地开发访问令牌JSON中读取的代码相同的代码在服务凭据JSON文件中读取。
...
读取服务凭据后，这些凭据用于生成JWT，然后与AdobeIMS API交换访问令牌，然后使用JWT作为Cloud Service访问AEM。

此示例应用程序基于Node.js，因此最好使用[@adobe/jwt-auth](https://www.npmjs.com/package/@adobe/jwt-auth) npm模块来促进(1)JWT生成和(20)与AdobeIMS交换。 如果您的应用程序是使用其他语言开发的，请查看[相应的代码示例](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/JWT/samples/samples.md)，了解如何使用其他编程语言构建AdobeIMS的HTTP请求。
...
1. 在进行这些更改后，从AEM开发人员控制台下载的服务凭据JSON（为简单起见，将其保存为与此`index.js`相同的文件夹`service_token.json`），执行将命令行参数`file`替换为`service_token.json`的应用程序，并将`propertyValue`更新为新值，以便在AEM中显示效果。
   ...
   __403 - Forbidden__&#x200B;行指示作为Cloud Service对AEM的HTTP API调用中的错误。 尝试更新资产的元数据时，会出现这403个禁止的错误。

   其原因是服务凭据派生访问令牌使用自动创建的技术帐户AEM用户对发往AEM的请求进行身份验证，默认情况下，该用户仅具有读取权限。 要向AEM提供应用程序写入访问权限，必须在AEM中授予与访问令牌关联的技术帐户AEM用户权限。

## 在AEM中配置访问权限

服务凭据派生的访问令牌使用技术帐户AEM用户，该用户具有成员身份，为参与者AEM用户组。

![服务凭据——技术帐户AEM用户](./assets/service-credentials/technical-account-user.png)

技术帐户AEM用户存在于AEM中(在具有访问令牌的第一个HTTP请求后)后，可以像管理其他AEM用户一样管理此AEM用户的权限。

1. 首先，打开从AEM Developer Console下载的服务凭据JSON，找到技术帐户的AEM登录名，并找到`integration.email`值，该值应类似于：`12345678-abcd-9000-efgh-0987654321c@techacct.adobe.com`。
1. 以AEM管理员身份登录到相应的AEM环境的作者服务
1. 导航到&#x200B;__工具__ > __安全__ > __用户__
1. 找到步骤1中标识的&#x200B;__登录名__&#x200B;的AEM用户，并打开其&#x200B;__属性__
1. 导航到&#x200B;__组__&#x200B;选项卡，并添加&#x200B;__DAM用户__&#x200B;组（作为对资产的写访问权限）
1. 点按&#x200B;__保存并关闭__

在AEM中拥有对资产具有写入权限的技术帐户的情况下，重新运行应用程序：
...
1. 作为已更新的Cloud Service环境登录AEM（使用`aem`命令行参数中提供的相同主机名）
1. 导航到&#x200B;__资产__ > __文件__
...
现在，我们已通过本地开发访问令牌以编程方式将AEM作为Cloud Service访问，并使用生产就绪型服务到服务访问令牌访问！
