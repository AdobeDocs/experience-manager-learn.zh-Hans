---
title: 在HTML5表单提交时触发AEM工作流 — 处理PDF提交
description: 处理HTML5/PDF表单提交
...
在本部分中，我们将创建一个在AEM Publish上运行的简单servlet来处理PDFform或HTML5表单提交。 此servlet向在AEM创作实例中运行的servlet发出HTTPPOST请求，该servlet负责将提交的数据另存为AEM创作存储库中的`nt:file`节点。

以下是处理PDF/HTML5表单提交的servlet的代码。 在此servlet中，我们对AEM Author实例中&#x200B;**/bin/startworkflow**&#x200B;上装载的servlet进行POST调用。 此servlet将表单数据保存在AEM作者的存储库中。


## AEM Publish servlet

以下代码处理PDF/HTML5表单提交。 此代码在发布实例上运行。
