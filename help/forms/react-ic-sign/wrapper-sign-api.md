---
title: 创建要呈现给用户进行签名的Web窗体
description: 创建AEM包以显示用例所需的Acrobat符号方法。
...
开发了一个自定义AEM包，以创建Web窗体并将其返回给最终用户

* [创建临时文档](https://secure.na1.echosign.com/public/docs/restapi/v6#!/transientDocuments/createTransientDocument). 通过此调用上传的文档称为临时文档，因为它仅在上传后7天内可用。 返回的临时文档ID可用于需要引用已上传文件的API调用。 临时文档请求是一个包含三部分的多部分请求，分别为文件名、mime类型和文件流。 在此请求中，您一次只能上传一个文件。
* [创建Web窗体](https://secure.na1.echosign.com/public/docs/restapi/v6#!/widgets/createWidget).这是用于创建新Web窗体的主端点。 在“活动”状态下创建Web窗体以立即托管Web窗体。
* [检索Web窗体](https://secure.na1.echosign.com/public/docs/restapi/v6#!/widgets/getWidgets).Retrieve的Web窗体用户。 然后，将此Web窗体呈现给调用应用程序，用于签署文档。

## 创建Acrobat Sign OSGi配置

Acrobat Sign REST API需要集成密钥以及与集成密钥关联的电子邮件。 这两个值将作为OSGi配置属性提供，如下所示
