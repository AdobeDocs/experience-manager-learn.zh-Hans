...
如果您的xdp不基于XSD，请按照以下步骤创建XSD（架构），您将基于该XSD（架构）构建自适应表单
...
在示例代码中，我们从请求对象中提取xdp名称和其他参数。 如果表单不基于XSD，则会创建要与xdp合并的xml文档。如果表单基于XSD，则只需从自适应表单提交数据中提取相应的节点，以生成要与xdp模板合并的xml文档。
...
1. [下载并安装自定义DocumentServices包](/help/forms/assets/common-osgi-bundles/AEMFormsDocumentServices.core-1.0-SNAPSHOT.jar). 这有一个Servlet，用于将数据与XDP模板合并并并流回PDF
...
1. 填写几个表单字段。
1. 单击下载PDF以获取PDF。 您可能需要等待几秒钟才能下载PDF。

>[!NOTE]
>
>您可以在 [基于xsd的自适应表单](http://localhost:4502/content/dam/formsanddocuments/two/jcr:content?wcmmode=disabled). 确保将相应的参数传递到位于irs clientlib中的streampdf.js中的后处理端点。
