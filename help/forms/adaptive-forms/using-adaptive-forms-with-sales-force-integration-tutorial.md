...
* 您需要拥有Salesforce帐户
* 您需要创建一个连接的应用程序。 列出了用于创建应用程序的官方文档表单Salesforce [此处](https://help.salesforce.com/articleView?id=connected_app_create.htm&amp;type=0).
* 为应用程序提供适当的OAuth范围（出于测试目的，我已选择所有可用的OAuth范围）
* 提供回调URL。 在我的案例中，回调URL是

   * 如果您使用 **AEM Forms 6.3**，回调URL为https://gbedekar-w7-1:6443/etc/cloudservices/fdm/createlead.html。 在此URL中创建的是我的表单数据模型的名称。
...
在此示例中，gbedekar -w7-1:6443是我的服务器名称以及AEM运行所在的端口。

创建已连接的应用程序后，请注意 **使用者密钥和密钥**. 在AEM Forms中创建数据源时，您需要这些设置。
...
* 通过键入https://导航到云服务&lt;servername>：&lt;serverport> /etc/cloudservices.html，例如https://gbedekar-w7-1:6443/etc/cloudservices.html
* 向下滚动至“表单数据模型”。
* 单击“显示配置”。
* 单击“+”以添加新配置
...
* **访问令牌Url - https://na5.salesforce.com/services/oauth2/token**
...
* 创建数据源
   ...
   * 授权范围： ** api chatter_api full id openid refresh_token visualforce web**
   * 身份验证处理程序：授权载体
   * 单击“连接到OAuth”按钮。 如果您看到任何错误，请查看前面的步骤，以确保正确输入了所有信息。


在使用SalesForce创建数据源后，您可以使用刚刚创建的数据源创建表单数据集成。 的文档链接是 [此处](https://helpx.adobe.com/experience-manager/6-4/forms/using/create-form-data-models.html)
