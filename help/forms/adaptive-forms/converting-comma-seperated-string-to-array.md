---
title: 在AEM Forms工作流中将逗号分隔的字符串转换为字符串数组
description: 当您的表单数据模型将字符串数组作为输入参数之一时，您需要在调用表单数据模型的提交操作之前，按照自适应表单的提交操作生成的数据。
...
与表单数据模型关联的其余端点的输入数据需要字符串数组，如此屏幕快照中所示。 流程步骤中的自定义代码会将提交的数据转换为正确的格式。

![fdm-string-array](assets/string-array-fdm.png)

我们会将JSON对象路径和元素名称传递到流程步骤。 流程步骤中的代码会将元素的逗号分隔值替换为字符串数组。
![过程步骤](assets/create-string-array.png)
