...
从AEM Forms 6.5版本开始，我们现在可以创建未绑定到特定自适应表单的工作流模型。 借助此功能，您现在可以创建一个工作流模型，该工作流模型可以在不同的自适应表单提交中调用。 借助此功能，您可以拥有一个通用工作流来处理所有自适应表单提交以供审阅和批准。
...
1. **已提交到工作流**  — 这意味着在提交自适应表单时，工作流引擎会从提交的数据中提取表单名称。 这是需要选择的选项
1. 在变量路径中可用 — 这意味着自适应表单是从工作流变量中提取的。以下屏幕快照显示了您需要为从自适应表单中解耦工作流选择的正确选项
