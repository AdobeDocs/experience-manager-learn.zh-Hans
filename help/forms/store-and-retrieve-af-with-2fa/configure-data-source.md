...
AEM可通过多种方式实现与外部数据库的集成。 数据库集成最常见和标准的做法之一是通过使用Apache Sling连接池化数据源配置属性 [configMgr](http://localhost:4502/system/console/configMgr).
...
以下数据库用于此用例。 数据库有一个名为的表 `formdatawithattachments` ，如下面的屏幕快照中所示的4列。
...
然后，此表单数据模型将用于 **MyAccountForm** 获取与应用程序id关联的电话号码。

## 后续步骤

[编写用于保存表单附件的代码](./store-form-attachments.md)
