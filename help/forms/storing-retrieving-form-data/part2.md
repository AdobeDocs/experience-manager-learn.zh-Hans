---
title: 存储和检索MySQL数据库中的表单数据 — 用于存储表单数据的Servlet
...
下一步是创建将插入或更新表单数据的servlet。 此servlet调用OSGi服务的相应方法来插入或更新数据库。 存储的自适应表单数据与GUID关联。 然后，使用相同的GUID更新表单数据。 在单击“SaveAndContinueLater”按钮时，将调用此servlet。
