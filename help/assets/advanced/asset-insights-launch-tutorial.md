---
title: 使用AEM Assets和Asset Launch设置Adobe分析
description: 在这段由5部分组成的视频系列中，我们将介绍通过Launch by Adobe部署的Experience Manager资产分析的设置和配置。
...
在这段由5部分组成的视频系列中，我们将介绍通过Asset Launch部署的Experience Manager的Adobe分析功能的设置和配置。
...
该视频使用核心组件v2.2.2，它不再是最新版本；在继续下一部分之前，请确保使用最新版本。
...
增强核心组件并将代理组件（示例图像组件）用于资产分析。 编辑内容页面模板策略，以便为引用站点启用示例图像组件。

>[!VIDEO](https://video.tv.adobe.com/v/25944?quality=12&learn=on)

>[!NOTE]
>
>图像核心组件包括禁用资产UUID（在JCR内创建的节点的唯一标识符值）跟踪以禁用UUID跟踪的功能

核心图像组件使用 ***data-asset-id*** 属性在父项中 &lt;div> 以启用/禁用此功能。 代理组件会使用以下更改覆盖核心组件。

* 删除 ***data-asset-id*** 来自image.html中 &lt;img> 元素的父div
* 添加 ***data-aem-asset-id*** 直接添加到image.html中的 &lt;img> 元素
...
* Analytics公司名称
* Analytics用户名
* 共享密钥(可以从 *Adobe Analytics >管理员>公司设置> Web服务*)。
...
添加Adobe Analytics扩展、创建页面加载规则，并将AEM与Launch与Adobe IMS技术帐户集成。
...
* **\&lt;code>assetAnalytics.core.assetClicked\&lt;code>** ：当为asset-DOM元素调度“click”事件时调用，这仅在asset-DOM元素具有锚标记作为具有有效外部“href”属性的父项时相关
...
* assetAnalytics.core.assetLoaded() ：在页面加载完成后调用，并触发所有可跟踪图像的资产展示次数
* 承载已加载资源列表的Analytics变量： **contextdata[&#39;c.a.assets.idList&#39;]**
* assetAnalytics.core.assetClicked() ：当资产DOM元素具有包含有效href值的锚点标记时调用。 单击资产后，将创建一个已单击资产ID作为其值的Cookie。**（Cookie名称：a.assets.clickedid）**
...
* [Launch Switch Chrome扩展](https://chrome.google.com/webstore/detail/launch-and-dtm-switch/nlgdemkdapolikbjimjajpmonpbpmipk?hl=en)
...
还可以使用以下Chrome扩展将DTM切换到调试模式： [Launch和DTM交换机](https://chrome.google.com/webstore/detail/launch-and-dtm-switch/nlgdemkdapolikbjimjajpmonpbpmipk?hl=en). 这使得查看是否存在与DTM部署相关的任何错误变得更加容易。 此外，您可以通过任何浏览器手动将DTM切换到调试模式 *开发人员工具 — > JS控制台* 添加以下代码片段：

## 第5部分：测试分析跟踪和同步洞察数据{#analytics-tracking-asset-insights}

配置AEM Asset Reporting同步作业调度程序和Assets Insights报表
