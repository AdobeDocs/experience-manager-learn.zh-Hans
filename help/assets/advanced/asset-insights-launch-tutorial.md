...
>[!CAUTION]
>
>确保为您的实施下载[最新版本的核心组件](https://github.com/adobe/aem-core-wcm-components)。
...
## 第2部分：为示例图像组件{#sample-image-component-asset-insights}启用资产分析跟踪
...
核心图像组件使用图像标记的父&lt;div>中的&#x200B;***data-asset-id***&#x200B;属性来启用/禁用此功能。 代理组件会通过以下更改覆盖核心组件。
...
* ***data-aem-asset-*** idand  ***data-trackable=&#39;true&#39;*** 将保留在同一节点级别

>[!NOTE]
>
>*data-aem-asset-id=&#39;image.UUID&#39;* 和 *data-trackable=&#39;true&#39;* 是资产展示次数需要存在的关键属性。对于资产点击分析，除了&lt;img>标记中存在的上述数据属性之外，父&lt;a>标记还必须具有有效的href值。

## 第三部分：Adobe Analytics — 创建报表包，启用实时数据收集和AEM Assets报表{#adobe-analytics-asset-insights}
...
* Analytics公司名称
* Analytics用户名
* 共享密钥(可从&#x200B;*Adobe Analytics >管理员>公司设置> Web服务*&#x200B;获取)。
...
* **\&lt;code>assetAnalytics.core.assetLoaded\&lt;/code>** &lt;code>&lt;code>:在为asset-DOM元素调度“load”事件时调用。&lt;/code>&lt;/code>
* **\&lt;code>assetAnalytics.core.assetClicked\&lt;/code>** &lt;code>&lt;code>:在为asset-DOM元素调度“click”事件时调用，仅当asset-DOM元素具有父标记且具有有效的外部“href”属性时，此事件才相关&lt;/code>&lt;/code>

最后，Pagetracker将初始化函数实施为。

* **\&lt;code>assetAnalytics.dispatcher.init()\&lt;/code>** &lt;code>&lt;code>:调用以初始化页面跟踪器组件。&lt;/code>&lt;/code> 必须先调用此ID，然后才能从网页生成任何资产分析事件（展示次数和/或点击次数）。
...
还可以使用以下Chrome扩展将DTM切换到调试模式：[启动和DTM交换机](https://chrome.google.com/webstore/detail/launch-and-dtm-switch/nlgdemkdapolikbjimjajpmonpbpmipk?hl=en)。 这样，便于查看是否存在与DTM部署相关的错误。 此外，您还可以通过添加以下代码片段，通过任何浏览器&#x200B;*开发人员工具 — > JS控制台*&#x200B;手动将DTM切换为调试模式：
