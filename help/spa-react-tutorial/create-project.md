---
title: SPA编辑器项目 | AEM SPA Editor和React快速入门
description: 了解如何将Adobe Experience Manager(AEM)Maven项目用作与AEM SPA编辑器集成的React应用程序的起点。
sub-product: 站点
...
了解如何将Adobe Experience Manager(AEM)Maven项目用作与AEM SPA编辑器集成的React应用程序的起点。
...
在本章中，将根据[AEM项目原型](https://github.com/adobe/aem-project-archetype)部署新的AEM项目。 AEM项目将通过React SPA的非常简单起点进行引导。 本章中使用的项目将用作实施WKND SPA的基础，并将在以后的章节中构建。
...
3. 从[AEM项目原型](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype/releases/tag/aem-project-archetype-14)生成AEM项目时，使用以下属性：
   ...
   | 软件包 | com.adobe.aem.guides.wknd.spa.react |
   | includeExamples | n |

   >[!NOTE]
   >
   > 请注意`frontendModule=react`属性。 这可告知AEM项目原型使用起始程序[React代码库](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend-react.html)引导项目以与AEM SPA编辑器一起使用。
...
5. 您应会看到`wknd-spa-react.all`、`wknd-spa-react.ui.apps`和`wknd-spa-react.ui.content`的三个包。
...
6. 您还应看到`spa.project.core`和`core.wcm.components`的多个包。 这些依赖项会自动包含在原型中。 有关[AEM核心组件的更多信息，请参阅此处](https://docs.adobe.com/content/help/zh-Hans/experience-manager-core-components/using/introduction.html)。

   `spa.project.core` 是生成SPA编辑器预期的JSON模型API所需的依赖项。
...
2. 选择&#x200B;**[!DNL us]** > **[!DNL en]** > **[!DNL WKND SPA React Home Page]**&#x200B;页面，然后单击菜单栏中的&#x200B;**[!UICONTROL 编辑]**&#x200B;按钮以打开该页面：
...
1. 在&#x200B;**[!UICONTROL 页面编辑器]**&#x200B;中，单击&#x200B;**[!UICONTROL 页面信息]**&#x200B;按钮> **[!UICONTROL 查看已发布的]**:
