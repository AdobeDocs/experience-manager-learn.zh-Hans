---
title: SPA Editor项目 | AEM SPA编辑器和React入门
description: 了解如何将Adobe Experience Manager(AEM)Maven项目用作与SPA Editor集成的React应用程序的起点。
sub-product: 站点
...
# SPA Editor项目{#spa-editor-project}

了解如何将Adobe Experience Manager(AEM)Maven项目用作与SPA Editor集成的React应用程序的起点。
...
在本章中，将根据[AEM项目Archetype](https://github.com/adobe/aem-project-archetype)部署新的AEM项目。 AEM项目将由React SPA的一个非常简单的起点启动。 本章中使用的项目将作为实施WKND SPA的基础，并将在今后各章中加以建立。
...
3. 从[AEM项目原型](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype/releases/tag/aem-project-archetype-14)生成AEM项目时使用了以下属性：
   ...
   >[!NOTE]
   >
   > 注意`frontendModule=react`属性。 这告诉AEM Project Archetype使用启动程序[React code base](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend-react.html)引导项目以与SPA Editor一起使用。
...
5. 您应当看到`wknd-spa-react.all`、`wknd-spa-react.ui.apps`和`wknd-spa-react.ui.content`的三个软件包。
...
6. 您还应当看到`spa.project.core`和`core.wcm.components`的几个包。 这些依赖项会自动包含在原型中。 有关[AEM核心组件的更多信息，请访问](https://docs.adobe.com/content/help/zh-Hans/experience-manager-core-components/using/introduction.html)。
...
1. 在&#x200B;**[!UICONTROL 页面编辑器]**&#x200B;中，单击&#x200B;**[!UICONTROL 页面信息]**&#x200B;按钮> **[!UICONTROL 视图作为已发布]**:
   ...
   这将打开一个查询参数`?wcmmode=disabled`的新选项卡，该选项卡会有效地关闭AEM编辑器：[http://localhost:4502/content/wknd-spa-react/us/en/home.html?wcmmode=disabled](http://localhost:4502/content/wknd-spa-react/us/en/home.html?wcmmode=disabled)
...
2. 视图页面的源，并注意未找到文本内容&#x200B;**[!DNL Hello World]**&#x200B;或任何其他内容。 相反，您应看到HTML，如下所示：
   ...
   `clientlib-react.min.js` 是加载到页面上并负责呈现内容的SPA。
...
5. 在新选项卡中，打开[http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json)
   ...
   在下一章中，我们将检查如何将此JSON内容从AEM组件映射到SPA组件，以形成SPA AEM编辑器体验的基础。
