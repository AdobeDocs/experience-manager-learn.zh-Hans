---
title: 扩展组件 | AEM SPA Editor和React快速入门
...
您始终可以在[GitHub](https://github.com/adobe/aem-guides-wknd-spa/tree/React/extend-component-solution)上查看完成的代码，或通过切换到分支`React/extend-component-solution`在本地签出代码。
...
2. 导航到`ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/components/card`并查看`.content.xml`文件。
...
6. 在到`ui.frontend`模块的IDE交换机中，导航到`ui.frontend/src/components/Card`:
   ...
   ![React组件启动](assets/extend-component/react-card-component-start.png)
...
8. Inspect方法`get imageContent()`:
   ...
   在本例中，我们选择通过从`Card`组件传递`this.props`来重新使用现有的React图像组件`Image`。 在教程的后面部分，将实施`get bodyContent()`方法以显示标题、日期和行动动员按钮。
...
6. 更新&#x200B;**资产元数据**&#x200B;选项卡，以添加&#x200B;**替换文本**&#x200B;和&#x200B;**标题**&#x200B;的值。
   ...
   当前，更新对话框后不显示其他更改。 要向React组件显示新字段，我们需要更新`Card`组件的Sling模型。
...
要最终将组件对话框中的值显示给React组件，我们需要更新Sling模型，该模型将填充`Card`组件的JSON。 我们还有机会实施两个业务逻辑：
...
1. 在`core/src/main/java/com/adobe/aem/guides/wknd/spa/react/core/models/Card.java`处打开文件`Card.java`。
...
2. 在界面中添加以下方法：
   ...
   这些方法将通过JSON模型API公开，并传递到React组件。
...
7. 实施在`Card.java`接口中定义的其他方法：
   ...
   >[!NOTE]
   >
   > 您可以在此处](https://github.com/adobe/aem-guides-wknd-spa/blob/React/extend-component-solution/core/src/main/java/com/adobe/aem/guides/wknd/spa/react/core/models/impl/CardImpl.java)查看已完成的CardImpl.java。[
...
9. 在以下位置查看JSON模型响应：[http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json)并搜索`wknd-spa-react/components/card`:
...
现在，JSON模型中已填充了`ctaLinkURL`、`ctaText`、`cardTitle`和`cardLastModified`的新属性，我们可以更新React组件以显示这些属性。
...
2. 在`ui.frontend/src/components/Card/Card.js`处打开`Card.js`。
3. 添加方法`get ctaButton()`以呈现操作调用：
...
4. 添加`get lastModifiedDisplayDate()`方法，将`this.props.cardLastModified`转换为表示日期的本地化字符串。
...
5. 更新`get bodyContent()`以显示`this.props.cardTitle`，并使用在上述步骤中创建的方法：
...
6. 已在`Card.scss`添加Sass规则以设置标题样式、操作调用和上次修改日期。 通过将以下行添加到文件顶部的`Card.js`来包含这些样式：
   ...
   >[!NOTE]
   >
   > 您可以在此处](https://github.com/adobe/aem-guides-wknd-spa/blob/React/extend-component-solution/ui.frontend/src/components/Card/Card.js)查看已完成的[React卡组件代码。
