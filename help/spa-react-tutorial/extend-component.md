...
2. 导航到`ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/components/card`并视图`.content.xml`文件。
   ...
   属性`sling:resourceSuperType`指向`wknd-spa-react/components/image`，表示`Card`组件将继承WKND SPA图像组件的所有功能。
...
要最终将组件对话框中的值暴露给React组件，我们需要更新Sling Model，该Sling Model为`Card`组件填充JSON。 我们还有机会实施两个业务逻辑：
...
2. 打开`ui.frontend/src/components/Card/Card.js`处的`Card.js`。
3. 添加方法`get ctaButton()`以呈现行动调用：
...
6. 已在`Card.scss`添加了Sass规则，以设置标题样式、行动动员和上次修改日期。 通过将以下行添加到文件顶部的`Card.js`，可以包括这些样式：
   ...
   >[!NOTE]
   >
   > 您可以视图完成的[在此反应卡组件代码](https://github.com/adobe/aem-guides-wknd-spa/blob/React/extend-component-solution/ui.frontend/src/components/Card/Card.js)。
